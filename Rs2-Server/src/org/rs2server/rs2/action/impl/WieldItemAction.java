...
public class WieldItemAction extends Action {
	...
	public void execute() {
		this.stop();
		Item item = getMob().getInventory() != null ? getMob().getInventory()
				.get(slot) : null;
		...
		if (item.getEquipmentDefinition() == null
				|| item.getEquipmentDefinition().getType() == null) {
		...
		if (BoundaryManager.isWithinBoundaryNoZ(getMob().getLocation(),
				"Fremennik Trials")) {
			if (type.getSlot() != Equipment.SLOT_WEAPON
					&& type.getSlot() != Equipment.SLOT_SHIELD
					&& type.getSlot() != Equipment.SLOT_ARROWS) {
				getMob().getActionSender()
						.sendMessage(
								"You can only wield a weapon, shield or ammunition inside the Fremennik Trials minigame.");
				return;
			}
		}
		if (item.getEquipmentDefinition() != null
				&& item.getEquipmentDefinition().getSkillRequirements() != null) {
			for (Skill skill : item.getEquipmentDefinition()
					.getSkillRequirements().keySet()) {
				if (getMob().getSkills().getLevelForExperience(skill.getId()) < item
						.getEquipmentDefinition().getSkillRequirement(
								skill.getId())
						&& getMob().getAttribute("sneaky") == null) { // sneaky
																		// --
																		// josh
					if (getMob().getActionSender() != null) {
						getMob().getActionSender()
						...
						String level = "a ";
						if (Skills.SKILL_NAME[skill.getId()].toLowerCase()
								.startsWith("a")) {
							level = "an ";
						}
		...
		try {
			if (type.getSlot() == 3 || type.getSlot() == 5) {
				if (type == EquipmentType.WEAPON_2H) {
					if (getMob().getEquipment().get(Equipment.SLOT_WEAPON) != null
						...
						if (getMob().getEquipment().get(Equipment.SLOT_WEAPON) != null) {
							getMob().getInventory().add(
									getMob().getEquipment().get(
											Equipment.SLOT_WEAPON), slot);
							getMob().getEquipment().set(Equipment.SLOT_WEAPON,
									null);
						}
						if (getMob().getEquipment().get(Equipment.SLOT_SHIELD) != null) {
							getMob().getInventory().add(
									getMob().getEquipment().get(
											Equipment.SLOT_SHIELD), slot - 1);
							getMob().getEquipment().set(Equipment.SLOT_SHIELD,
									null);
						}
						...
						return;
					} else if (getMob().getEquipment().get(
							Equipment.SLOT_SHIELD) != null
							&& getMob().getEquipment().get(
									Equipment.SLOT_WEAPON) == null) {
						getMob().getInventory().remove(item, slot);
						getMob().getEquipment()
								.set(Equipment.SLOT_WEAPON, item);
						getMob().getInventory().add(
								getMob().getEquipment().get(
										Equipment.SLOT_SHIELD), slot);
						getMob().getEquipment()
								.set(Equipment.SLOT_SHIELD, null);
						getMob().getCombatState().calculateBonuses();
						if (getMob().getActionSender() != null) {
							getMob().getActionSender().sendBonuses();
						}
				...
				if (type.getSlot() == Equipment.SLOT_SHIELD
						...
						&& getMob().getEquipment().get(Equipment.SLOT_WEAPON)
								.getEquipmentDefinition() != null
						&& getMob().getEquipment().get(Equipment.SLOT_WEAPON)
								.getEquipmentDefinition().getType() == EquipmentType.WEAPON_2H) {
					getMob().getInventory().remove(item, slot);
					getMob().getInventory().add(
							getMob().getEquipment().get(Equipment.SLOT_WEAPON),
							slot);
			...
			if (getMob().getEquipment().get(type.getSlot()) != null) {
				if (getMob().getEquipment().get(type.getSlot()).getId() == item
						.getId() && item.getDefinition().isStackable()) {
					item = new Item(item.getId(), getMob().getEquipment()
							.get(type.getSlot()).getCount()
							+ item.getCount());
				} else {
					if (getMob().getEquipment().get(type.getSlot())
							.getEquipmentDefinition() != null) {
						for (int i = 0; i < getMob().getEquipment()
								.get(type.getSlot()).getEquipmentDefinition()
								.getBonuses().length; i++) {
							getMob().getCombatState().setBonus(
									i,
									getMob().getCombatState().getBonus(i)
											- getMob().getEquipment()
													.get(type.getSlot())
					...
					getMob().getInventory().add(
							getMob().getEquipment().get(type.getSlot()), slot);
				}
			}
			getMob().getEquipment().set(type.getSlot(), item);
			if (item.getEquipmentDefinition() != null) {
