...
public abstract class ObjectHarvestingAction extends Action {
	 ...
	 */
	public ObjectHarvestingAction(Mob mob) {
		super(mob, 0);
	}
	...
	public void execute() {
		...
		} // decreases the health by one, e.g. lost a log
		...
		if (getGameObject().getCurrentHealth() < 1) {
			getGameObject().setCurrentHealth(getGameObjectMaxHealth()); // sets
																		// its
																		// health
																		// back
																		// to
			...
			World.getWorld().replaceObject(getGameObject(),
					getReplacementObject(), getObjectRespawnTimer());

			getMob().playAnimation(Animation.create(-1));
			this.stop(); // stops the action as the object is completely
							// harvested.
			return;
		}
		if (!getMob().getInventory().hasRoomFor(getReward())) {
	 ...
	 *         object.
	 */
	public abstract String getUnsuccessfulAttemptMessage();
}
