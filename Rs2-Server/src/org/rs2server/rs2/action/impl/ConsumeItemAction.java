...
public class ConsumeItemAction extends Action {
	...
	public void execute() {
		...
		boolean inventoryFiringEvents = getMob().getInventory()
				.isFiringEvents();
		...
		try {
			...
			if (food != null && getMob().getCombatState().canEat()) {
				...
				getMob().getActionSender().playSound(
						Sound.create(2393, (byte) 1, 0));
				/**
				 ...
				 */
				if (getMob().getCombatState().getAttackDelay() + 3 <= getMob()
						.getCombatCooldownDelay() + 3) {
				...
				int increasedBy = 0;
				int beforeHitpoints = getMob().getSkills().getLevel(
						Skills.HITPOINTS);

				getMob().getSkills().increaseLevelToMaximum(Skills.HITPOINTS,
						food.getHeal());

				increasedBy = getMob().getSkills().getLevel(Skills.HITPOINTS)
						- beforeHitpoints;
				if (increasedBy > 0) {
					/**
				 ...
				 */
				if (food.getNewId() != -1) {
					getMob().getInventory().add(new Item(food.getNewId(), 1),
							slot);
				}
				 ...
				 * Potion Types.
				 */
				String potionName = item
						.getDefinition()
						.getName()
						.toLowerCase()
						.substring(0,
								item.getDefinition().getName().length() - 3)
						.replaceAll(" potion", "");
				switch (drink.getPotionType()) {
				case NORMAL_POTION:
					getMob().getActionSender()
							.sendMessage(
									"You drink some of your " + potionName
											+ " potion.");
					for (int i = 0; i < drink.getSkills().length; i++) {
						int skill = drink.getSkill(i);
						int modification = (int) Math
								.floor((drink == Drink.RANGE_POTION ? 4 : 3)
										+ (getMob().getSkills()
												.getLevelForExperience(skill) * 0.1));
						getMob().getSkills()
								.increaseLevelToMaximumModification(skill,
										modification);
					}
					break;
				case SUPER_POTION:
					...
					for (int i = 0; i < drink.getSkills().length; i++) {
						int skill = drink.getSkill(i);
						int modification = (int) Math
								.floor(5 + (getMob().getSkills()
										.getLevelForExperience(skill) * 0.15));
						getMob().getSkills()
								.increaseLevelToMaximumModification(skill,
										modification);
					}
				...
				case PRAYER_POTION:
					getMob().getActionSender().sendMessage(
							"You drink some of your restore prayer potion.");
					for (int i = 0; i < drink.getSkills().length; i++) {
						int skill = drink.getSkill(i);
						int modification = (int) Math
								.floor(7 + (getMob().getSkills()
										.getLevelForExperience(skill) * 0.25));
						/**
						...
						if (skill == Skills.PRAYER) {
							if (getMob().getInventory().contains(6714)) {
								modification++;
								if (getMob().getSkills().getLevelForExperience(
										Skills.PRAYER) >= 40) {
									modification++;
								}
								if (getMob().getSkills().getLevelForExperience(
							...
							getMob().getSkills().increasePrayerPoints(
									modification);
						} else {
							getMob().getSkills().increaseLevelToMaximum(skill,
									modification);
						}
					}
					break;
				...
				case SUPER_RESTORE:
					...
					for (int i = 0; i < drink.getSkills().length; i++) {
						int skill = drink.getSkill(i);
						int modification = getMob().getSkills()
								.getLevelForExperience(skill) / 3;
						/**
						 * Holy wrench increases prayer restoration.
						 */
						if (skill == Skills.PRAYER) {
				...
				case SARADOMIN_BREW:
					...
					for (int i = 0; i < drink.getSkills().length; i++) {
						...
						if (skill == Skills.HITPOINTS) {
							int hitpointsModification = (int) (getMob()
									.getSkills().getLevelForExperience(
											Skills.HITPOINTS) * 0.15);
							getMob().getSkills()
									.increaseLevelToMaximumModification(skill,
											hitpointsModification);
						} else if (skill == Skills.DEFENCE) {
							int defenceModification = (int) (getMob()
									.getSkills().getLevelForExperience(
											Skills.DEFENCE) * 0.25);
							getMob().getSkills()
									.increaseLevelToMaximumModification(skill,
							...
							int modification = (int) (getMob().getSkills()
									.getLevelForExperience(skill) * 0.10);
							getMob().getSkills().decreaseLevelOnce(skill,
									modification);
						}
					}
					break;
					...
					for (int i = 0; i < drink.getSkills().length; i++) {
						...
						if (skill == Skills.ATTACK) {
							int attackModification = (int) Math
									.floor(2 + (getMob().getSkills()
											.getLevelForExperience(Skills.ATTACK)) * 0.20);
							getMob().getSkills()
									.increaseLevelToMaximumModification(skill,
											attackModification);
						} else if (skill == Skills.STRENGTH) {
							int strengthModification = (int) Math
									.floor(2 + (getMob().getSkills()
											.getLevelForExperience(
													Skills.STRENGTH) * 0.12));
							getMob().getSkills()
									.increaseLevelToMaximumModification(skill,
											strengthModification);
						} else if (skill == Skills.PRAYER) {
							int prayerModification = (int) Math.floor(getMob()
									.getSkills().getLevelForExperience(
											Skills.STRENGTH) * 0.10);
							getMob().getSkills().increaseLevelToMaximum(skill,
									prayerModification);
						} else if (skill == Skills.DEFENCE) {
							int defenceModification = (int) Math
									.floor(2 + (getMob().getSkills()
											.getLevelForExperience(
													Skills.DEFENCE) * 0.10));
							getMob().getSkills().decreaseLevelToZero(skill,
									defenceModification);
							...
							World.getWorld().submit(new Tickable(3) {
								...
								public void execute() {
									int hitpointsModification = (int) Math
											.floor(2 + (getMob().getSkills()
													.getLevel(Skills.HITPOINTS) * 0.10));
									if (getMob().getSkills().getLevel(
											Skills.HITPOINTS)
											- hitpointsModification < 0) {
										hitpointsModification = getMob()
												.getSkills().getLevel(
														Skills.HITPOINTS);
									}
									getMob().inflictDamage(
											new Hit(hitpointsModification),
											null);
									this.stop();
								}
							});
				...
				case SUPER_ANTIPOISON:
					...
					if (getMob().getCombatState().canBePoisoned()) {
						...
						World.getWorld()
								.submit(new Tickable(
										drink.getPotionType() == PotionType.ANTIPOISON ? 150
												: 1000) {
									@Override
									public void execute() {
										getMob().getCombatState()
				...
				case BEER:
					...
					for (int i = 0; i < drink.getSkills().length; i++) {
						int skill = drink.getSkill(i);
						if (skill == Skills.ATTACK) {
							int attackModification = (int) (getMob()
									.getSkills().getLevelForExperience(
											Skills.STRENGTH) * 0.07);
							getMob().getSkills().decreaseLevelToZero(
									Skills.ATTACK, attackModification);
						} else if (skill == Skills.STRENGTH) {
							int strengthModification = (int) (getMob()
									.getSkills().getLevelForExperience(
											Skills.STRENGTH) * 0.04);
							getMob().getSkills()
									.increaseLevelToMaximumModification(
											Skills.STRENGTH,
											strengthModification);
						}
					}
				...
				case WINE:
					...
					for (int i = 0; i < drink.getSkills().length; i++) {
						...
						if (skill == Skills.ATTACK) {
							...
							getMob().getSkills().increaseLevelToMaximum(
									Skills.HITPOINTS, 11);
						}
					}
					break;
				...
				if (drink.getPotionType() != PotionType.BEER
						&& drink.getPotionType() != PotionType.WINE) {
					getMob().getActionSender()
							.sendMessage(
									currentPotionDose > 1 ? ("You have "
											+ (currentPotionDose - 1)
											+ " dose"
