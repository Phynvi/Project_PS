...
public class Cannon {
	...
	private enum FacingState {
		NORTH(0, 515), NORTH_EAST(1, 516), EAST(2, 517), SOUTH_EAST(3, 518), SOUTH(
				4, 519), SOUTH_WEST(5, 520), WEST(6, 521), NORTH_WEST(7, 514);
	...
	public void addPart(Item item) {
		...
		if (id != -1) {
			...
			this.gameObject = new GameObject(gameObject.getLocation(), id, 10,
					0, false);
			World.getWorld().register(this.gameObject);
			partsAdded.add(item);
		}
			...
			if (!player.getInventory().add(item)) {
				if (player.getBank().add(item)) {
					player.getActionSender().sendMessage(
							"You don't have enough inventory space to pick up the "
	...
	public void fire() {
		...
		if (cannonBalls < 1) {
			player.getActionSender().sendMessage(
					"There are no cannonballs currently loaded.");
			return;
		}
			...
			public void execute() {
				...
				for (Region r : gameObject.getRegion().getSurroundingRegions()) {
					for (final NPC npc : r.getNpcs()) {
						...
						int newDist = gameObject.getLocation()
								.distanceToEntity(gameObject, npc);
						if (newDist <= 5 && newDist >= 1) {
							...
							if (player.getActiveCombatAction().canHit(player,
									npc, false, true)) {
								gameObject.playProjectile(Projectile.create(
										gameObject.getCentreLocation(),
										npc.getCentreLocation(), 53,
										15 + (delay * 10), 50,
										50 + (newDist * 10), 37, 37,
										npc.getProjectileLockonIndex(), 0, 96));
								cannonBalls--;
								delay += 1;
										...
										player.getSkills()
												.addExperience(
														...
														(4 * damage)
																* Constants
																		.getExpModifier());
										npc.inflictDamage(new Hit(damage),
												player);
										npc.getActiveCombatAction().defend(
												player, npc, true);
										this.stop();
									}
								});
