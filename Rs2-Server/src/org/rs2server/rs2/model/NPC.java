...
public class NPC extends Mob {
	 ...
	 *            The definition.
	 */
	public NPC(NPCDefinition definition, Location spawnLocation,
		...
		World.getWorld().submit(new Task() {
			...
			public void execute(GameEngine context) {
				...
				for (int i = 0; i < constantDrops.length; i++) {
					if (constantDrops[i].getFrequency() >= 1 && mob.isPlayer()) {
						World.getWorld().createGroundItem(
								new GroundItem(mob.getUndefinedName(),
										constantDrops[i].getItem(),
										getLocation()), (Player) mob);
						System.out.println("Dropped item "
								+ constantDrops[i].getItem().toString()
								+ " at frequency "
								+ constantDrops[i].getFrequency());
					}
				}
				...
				if (randomDrops.length > 0) {
					NPCDrop drop = null;
					while (drop == null) {
						NPCDrop randomDrop = randomDrops[random
								.nextInt(randomDrops.length)];
						boolean alwaysDrop = randomDrop.getFrequency() >= 1;
						if (!alwaysDrop
								&& random.nextDouble() < randomDrop
										.getFrequency()) {
							drop = randomDrop;
						}
					}
					System.out.println("Dropped item "
							+ drop.getItem().toString() + " at frequency "
							+ drop.getFrequency());
					World.getWorld().createGroundItem(
							new GroundItem(mob.getUndefinedName(), drop
									.getItem(), getLocation()), (Player) mob);
				}
	...
	public Location getCentreLocation() {
		return Location.create(
				getLocation().getX() + (int) Math.floor(getWidth() / 2),
				getLocation().getY() + (int) Math.floor(getHeight() / 2),
				getLocation().getZ());
	}
