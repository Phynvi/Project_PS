...
import org.rs2server.rs2.model.quest.QuestStorage;
import org.rs2server.rs2.model.region.Region;
import org.rs2server.rs2.model.skills.SlayerTask;
...
public class Player extends Mob implements Persistable {
	 ...
	 * The Music manager
	 */
	private final Music music = new Music(this);
	/**
	...
	private int pizazzPoints = 0;
	private int barrowsKilled = 0;
	/*
	 * Minigame details
	 */
	/**
		...
		if (victim.isPlayer()) {
			if (!BoundaryManager.isWithinBoundaryNoZ(getLocation(), "PvP Zone")) {
				if (messages) {
					getActionSender()
							.sendMessage(
									"You need to be within the wilderness to attack another player.");
				}
				return false;
			} else if (!BoundaryManager.isWithinBoundaryNoZ(
					victim.getLocation(), "PvP Zone")) {
				if (messages) {
					getActionSender()
							.sendMessage(
									"You cannot attack another player outside of the wilderness");
				}
				return false;
			}
	...
	public void dropLoot(Mob mob) {
		...
		try {
			...
			World.getWorld().createGroundItem(
					new GroundItem(player.getName(), new Item(526),
							getLocation()), player);
	...
	public int getDaysOfMembership() {
		return TextUtils.toJagexDateFormatCeil(membershipExpiryDate
				- System.currentTimeMillis()); // flagged as even 1 second into
												// a
		...
		for (int i = 0; i < Inventory.SIZE; i++) {
			...
			if (item != null) {
				...
				for (int k = 0; k < count; k++) {
					...
					if (topItem == null
							|| item.getDefinition().getValue() > topItem
	...
	/**
	 * @param fremennikTrials
	 *            the fremennikTrials to set
	 */
