...
public class Trade {
	...
	public static void acceptTrade(Player player, int screenStage) {
		...
		case 1:
			if (player.getInventory().freeSlots() < partner.getTrade().size()) {
				player.getActionSender().sendMessage(
						"You do not have enough free inventory slots.");
				player.getActionSender().sendString(TRADE_INVENTORY_INTERFACE,
						56, "");
				partner.getActionSender().sendString(TRADE_INVENTORY_INTERFACE,
						56, "");
				player.getRequestManager().setState(RequestState.PARTICIPATING);
				partner.getRequestManager()
						.setState(RequestState.PARTICIPATING);
				return;
			}
			for (Item item : partner.getTrade().toArray()) {
				if (item != null && item.getDefinition().isStackable()
						&& player.getInventory().getCount(item.getId()) > 0) {
					long partnerCount = player.getInventory().getCount(
							item.getId());
					long myCount = item.getCount();
					long totalCount = (partnerCount + myCount);
					if (totalCount > Integer.MAX_VALUE) {
						player.getActionSender().sendMessage(
								"You cannot accept this amount of "
										+ item.getDefinition().getName()
										+ (item.getDefinition().getName()
												.endsWith("s") ? "" : "s")
										+ ".");
						player.getActionSender().sendString(
								TRADE_INVENTORY_INTERFACE, 56, "");
						partner.getActionSender().sendString(
								TRADE_INVENTORY_INTERFACE, 56, "");
						player.getRequestManager().setState(
								RequestState.PARTICIPATING);
						partner.getRequestManager().setState(
								RequestState.PARTICIPATING);
						return;
					}
				}
			...
			if (partner.getInventory().freeSlots() < player.getTrade().size()) {
				player.getActionSender()
						.sendMessage(
								"The other player does not have enough free inventory slots.");
				player.getActionSender().sendString(TRADE_INVENTORY_INTERFACE,
						56, "");
				partner.getActionSender().sendString(TRADE_INVENTORY_INTERFACE,
						56, "");
			...
			for (Item item : player.getTrade().toArray()) {
				if (item != null && item.getDefinition().isStackable()
						&& partner.getInventory().getCount(item.getId()) > 0) {
					long partnerCount = partner.getInventory().getCount(
							item.getId());
					long myCount = item.getCount();
					long totalCount = (partnerCount + myCount);
					if (totalCount > Integer.MAX_VALUE) {
						player.getActionSender().sendMessage(
								"The other player cannot accept this amount of "
										+ item.getDefinition().getName()
										+ (item.getDefinition().getName()
												.endsWith("s") ? "" : "s")
										+ ".");
		...
		case 2:
			if (partner.getRequestManager().getState() == RequestState.CONFIRM_2) {
				...
				player.getActionSender().sendString(SECOND_TRADE_SCREEN, 33,
						"Waiting for other player...");
				partner.getActionSender().sendString(SECOND_TRADE_SCREEN, 33,
						"Other player has accepted");
			}
			break;
		}
	 ...
	 * 
	 * @param player
	 *            The player to open the trade for.
	 */
	public static void open(Player player, Player partner) {
		player.getActionSender()
				.sendRunScript(150, Constants.TRADE_PARAMETERS_2,
						Constants.TRADE_TYPE_STRING)
				.sendRunScript(150, Constants.TRADE_PARAMETERS_1, "iiiiii")
				.sendRunScript(150, Constants.OFFER_PARAMETERS,
						Constants.TRADE_TYPE_STRING)
				...
				.sendString(
						...
						20,
						partner.getName() + " has "
								+ partner.getInventory().freeSlots()
								+ " free inventory slots.");
		player.getInterfaceState().addListener(player.getTrade(),
				new InterfaceContainerListener(player, -1, 1, 81));
		...
		player.getInterfaceState().addListener(
				partner.getTrade(),
				new InterfaceContainerListener(player, -2,
						TRADE_INVENTORY_INTERFACE << 16 | 50, 80));
		...
		partner.getActionSender()
				...
				.sendString(
						...
						player.getName() + " has "
								...
								+ " free inventory slots.");
		partner.getInterfaceState().addListener(partner.getTrade(),
				new InterfaceContainerListener(partner, -1, 1, 81));
		partner.getInterfaceState().addListener(partner.getInventory(),
				new InterfaceContainerListener(partner, -1, 1, 82));
		partner.getInterfaceState().addListener(
				player.getTrade(),
				new InterfaceContainerListener(partner, -2,
						TRADE_INVENTORY_INTERFACE << 16 | 50, 80));
	}
	...
	public static void removeItem(Player player, int slot, int id, int amount) {
		...
		try {
			...
			if (player.getInventory().add(
				...
				player.getTrade()
						.remove(new Item(item.getId(), transferAmount));
			}
			player.getInventory().fireItemsChanged();
		} finally {
			player.getInventory().setFiringEvents(inventoryFiringEvents);
	...
	public static void secondScreen(Player player) {
		...
		if (!myOneLine) {
			...
			String firstHalfString = listContainerContents(firstHalf).replace(
					"<col=FFFFFF>Absolutely nothing!", "");
			String secondHalfString = listContainerContents(secondHalf)
					.replace("<col=FFFFFF>Absolutely nothing!", "");
			player.getActionSender().sendInterfaceConfig(334, 38, false)
					// Thin text left - Your offer
					.sendInterfaceConfig(334, 39, false)
					// Thin text right - Your offer
					.sendInterfaceConfig(334, 37, true)
					// Bold text centre - Your offer
					.sendString(334, 38, firstHalfString)
					.sendString(334, 39, secondHalfString);

			partner.getActionSender().sendInterfaceConfig(334, 41, false)
					// Thin text left - Opponents offer
					.sendInterfaceConfig(334, 42, false)
					// Thin text right - Opponents offer
					.sendInterfaceConfig(334, 40, true)
					// Bold text centre - Opponents offer
					.sendString(334, 41, firstHalfString)
					.sendString(334, 42, secondHalfString);
		} else {
			player.getActionSender()
					.sendInterfaceConfig(334, 38, true)
					// Thin text left - Your offer
					.sendInterfaceConfig(334, 39, true)
					// Thin text right - Your offer
					.sendInterfaceConfig(334, 37, false)
					// Bold text centre - Your offer
					.sendString(334, 37,
							listContainerContents(player.getTrade()));

			partner.getActionSender()
					.sendInterfaceConfig(334, 41, true)
					// Thin text left - Opponents offer
					.sendInterfaceConfig(334, 42, true)
					// Thin text right - Opponents offer
					.sendInterfaceConfig(334, 40, false)
					// Bold text centre - Opponents offer
					.sendString(334, 40,
							listContainerContents(player.getTrade()));
		}
		if (!otherOneLine) {
			...
			partner.getActionSender().sendInterfaceConfig(334, 38, false)
					// Thin text left - Your offer
					.sendInterfaceConfig(334, 39, false)
					// Thin text right - Your offer
					.sendInterfaceConfig(334, 37, true)
			...
			player.getActionSender()
					...
					// Bold text centre - Opponents offer
					.sendString(334, 40,
							listContainerContents(partner.getTrade()));
		...
		player.getActionSender()
				.sendInterface(SECOND_TRADE_SCREEN, false)
				.sendString(
						334,
						44,
		...
		// .sendInterfaceConfig(334, 40, false) // Bold text centre - Opponents
		// offer
		// .sendInterfaceConfig(334, 41, false) // Thin text left - Opponents
		// offer
		// .sendInterfaceConfig(334, 42, false) // Thin text right - Opponents
		// offer
		// .sendInterfaceConfig(334, 45, false) // Trade modified - Your offer
		// .sendInterfaceConfig(334, 46, false) // Trade modified - Opponents
		// offer
		...
		// if (player.getTrade().get(i).getCount() >= 1000 &&
		// player.getTrade().get(i).getCount() < 1000000) {
		// tradeStringAmt = "@cya@" + (player.getTrade().get(i).getCount() /
		// 1000) + "K @whi@(" +
		// NumberFormat.getInstance().format(player.getTrade().get(i).getCount())
		// + ")";
		// } else if (player.getTrade().get(i).getCount() >= 1000000) {
		// tradeStringAmt = "@gre@" + (player.getTrade().get(i).getCount() /
		// 1000000) + " million @whi@(" +
		// NumberFormat.getInstance().format(player.getTrade().get(i).getCount())
		// + ")";
		// } else {
		...
		// }
		// if
		// (ItemDefinition.forId(player.getTrade().get(i).getId()).isStackable())
		// {
		...
		// if (partner.getTrade().get(i).getCount() >= 1000 &&
		// partner.getTrade().get(i).getCount() < 1000000) {
		// tradeStringAmt = "@cya@" + (partner.getTrade().get(i).getCount() /
		// 1000) + "K @whi@(" +
		// NumberFormat.getInstance().format(partner.getTrade().get(i).getCount())
		// + ")";
		// } else if (partner.getTrade().get(i).getCount() >= 1000000) {
		// tradeStringAmt = "@gre@" + (partner.getTrade().get(i).getCount() /
		// 1000000) + " million @whi@(" +
		// NumberFormat.getInstance().format(partner.getTrade().get(i).getCount())
		// + ")";
		// } else {
		...
		// }
		// if
		// (ItemDefinition.forId(partner.getTrade().get(i).getId()).isStackable())
		// {
		// tradeString = tradeString + " x " + tradeStringAmt;
		// }
		//
	...
	public static void updateFirstScreen(Player player) {
		...
		partner.getActionSender().sendString(
				335,
				20,
				player.getName() + " has " + player.getInventory().freeSlots()
						+ " free inventory slots.");
	}
}
