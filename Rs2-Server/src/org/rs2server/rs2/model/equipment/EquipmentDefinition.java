...
public class EquipmentDefinition {
	...
	public enum Skill {
		...
		ATTACK(Skills.ATTACK), DEFENCE(Skills.DEFENCE), HITPOINTS(
				Skills.HITPOINTS), RANGE(Skills.RANGE), MAGIC(Skills.MAGIC), PRAYER(
				Skills.PRAYER), STRENGTH(Skills.STRENGTH), SLAYER(Skills.SLAYER);
		/**
		 * The list of skills.
		 */
		private static Map<Integer, EquipmentDefinition.Skill> skills = new HashMap<Integer, EquipmentDefinition.Skill>();

		/**
		 * Populates the skill list.
		 */
		static {
			for (EquipmentDefinition.Skill skill : EquipmentDefinition.Skill
					.values()) {
				skills.put(skill.getId(), skill);
			}
		}

		public static EquipmentDefinition.Skill skillForId(int skill) {
	...
	private static EquipmentAnimations DEFAULT_ANIMATIONS = new EquipmentAnimations(
			Animation.create(808), Animation.create(819),
			Animation.create(824), new Animation[] { Animation.create(422),
					Animation.create(423), Animation.create(422),
					Animation.create(422) }, Animation.create(424),
			Animation.create(823), Animation.create(820),
	...
	private static final Logger logger = Logger
			.getLogger(EquipmentDefinition.class.getName());
	/**
	 * We shall allocate a size for the HashMap!
	 */
	public static final int ALLOCATED_SIZE = 2385;
	...
	public static void init() throws IOException {
		if (definitions != null) {
			throw new IllegalStateException(
					"Equipment definitions already loaded.");
		}
		...
		try {
			 ...
			 */
			definitions = new HashMap<Integer, EquipmentDefinition>(
					ALLOCATED_SIZE);
		 ...
		 * if(itemDef == null) { continue; } EquipmentDefinition def =
		 * definitions.get(itemDef.getId()); if(def != null) {
		 * if(itemDef.getId() == 4734) { definitions.put(itemDef.getId(), new
		 * EquipmentDefinition(3, def.getBonuses(), def.getWeaponStyles(),
		 * def.getType(), def.getAnimation(), def.getPoisonType(),
		 * def.getSkillRequirements(), def.getSpecialConsumption(),
		 * def.getSpecialHits())); } } } System.out.println("Dumped ");
		 * XMLController.writeXML(definitions, new
		 * File("data/equipmentDefinition.xml")); /*please leave incase I need
		 * to redump any invalid values, thanks.
		 */
	...
	public static void setDefinitions(
			Map<Integer, EquipmentDefinition> definitions) {
	 ...
	 */
	private Map<EquipmentDefinition.Skill, Integer> skillRequirements;
	/**
	...
	public int getSkillRequirement(int index) {
		return skillRequirements.get(EquipmentDefinition.Skill
				.skillForId(index));
	}
	 ...
	 * @return The weapons skill requirements.
	 */
	public Map<EquipmentDefinition.Skill, Integer> getSkillRequirements() {
