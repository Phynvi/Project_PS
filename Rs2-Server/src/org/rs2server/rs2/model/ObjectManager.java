...
import org.rs2server.cache.map.MapListener;
import org.rs2server.cache.map.MapParser;
...
public class ObjectManager implements LandscapeListener,
		ObjectDefinitionListener, MapListener {
	...
	public void load() throws IOException, InvalidCacheException {
		...
		try {
			...
			MapParser.loadTileRegions(this);
			ObjectDef.loadConfig();
			RegionClipping.getRegionClipping().load(this);
			/*
			 * MapIndex[] mapIndices = cache.getIndexTable().getMapIndices();
			 * for(MapIndex index : mapIndices) { new LandscapeParser(cache,
			 * index.getIdentifier(), this).parse(); }
			 */
			int customObjectCount = 0;
			List<GameObject> customObjects = XMLController.readXML(new File(
					"data/customObjects.xml"));
			for (GameObject obj : customObjects) {
				World.getWorld().register(
						new GameObject(obj.getLocation(), obj.getId(), obj
								.getType(), obj.getDirection(), obj
								.isLoadedInLandscape()));
				customObjectCount++;
			}
			logger.info("Loaded " + objectCount + " objects and "
					+ customObjectCount + " custom objects.");
		} finally {
			cache.close();
		}
	}
	...
	@Override
	public void tileParsed(Tile tile, Location location) {
