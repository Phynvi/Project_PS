...
public class Shop {
	...
	public static enum ShopType {
		...
		/**
		 * A general store, which takes all items, buys items for 0.4 of their
		 * price and sells them for 0.8 of their price. Default stock will go to
		 * 0, and non-default will be removed if the stock is < 1.
		 */
		GENERAL_STORE,

		/**
		 * A specialist store that does buy items that are in its stock (none
		 * others though!). It will buy items for 0.6 of their price, and sell
		 * them for 1.0
		 */
		SPECIALIST_STORE_BUY,

		/**
		 * A specialist store that will not buy any stock. It will sell items
		 * for 1.0 of their price.
		 */
	...
	public static void buyItem(Player player, int slot, int id, int amount) {
		...
		Item item = player.getInterfaceState().getOpenStockType() == 1 ? shop
				.getMainStock().get(slot) : shop.getPlayerStock().get(slot);
		...
		if (transferAmount > 200) {
			...
			player.getActionSender().sendMessage(
					"You cannot buy more than 200 items at a time.");
		}
		...
		case 1: // Main stock
			if (shop.getMainStock().get(slot).getCount() > 0) {
				...
				if (!shop.hasCurrency(player, item, amount)) {
					...
					temporaryInventory.remove(new Item(shop.getCurrency(),
							amount
									* shop.getCostValue(player,
											new Item(item.getId(), amount))));
					if (!temporaryInventory.add(reward)) {
						return; // We wouldn't have enough inventory space, even
								// after removing the currency cost.
					}
					player.getInventory().remove(
							new Item(shop.getCurrency(), amount
									* shop.getCostValue(player,
											new Item(item.getId(), amount))));
		...
		case 2: // Player stock
			if (shop.getPlayerStock().get(slot).getCount() > 0) {
				...
				if (!shop.hasCurrency(player, item, amount)) {
					...
					long totalValue = shop.getCostValue(player,
							new Item(item.getId(), amount));
					long totalPrice = (totalAmount * totalValue);
					if (totalPrice > Integer.MAX_VALUE
							|| totalPrice > player.getInventory().getCount(
									shop.getCurrency())) {
						amount = (player.getInventory().getCount(
								shop.getCurrency()) / shop.getSellValue(player,
								new Item(item.getId(), amount))) - 1;
					}
					Item reward = new Item(item.getId(), amount);
					Container temporaryInventory = new Container(Type.STANDARD,
							Inventory.SIZE);
					...
					player.getInventory().remove(
							new Item(shop.getCurrency(), amount
									* shop.getCostValue(player,
											new Item(item.getId(), amount))));
					player.getInventory().add(reward);
					if (shop.getDefaultStock().contains(item.getId())) { // If
																			// the
																			// player
																			// stock
																			// started
																			...
																			// removing
																			// completely.
						shop.getPlayerStock().removeOrZero(
								new Item(item.getId(), amount));
					} else {
						shop.getPlayerStock().remove(
								new Item(item.getId(), amount));
					}
				}
			}
	...
	public static void costItem(Player player, int slot, int id) {
		...
		player.getActionSender().sendMessage(
				ItemDefinition.forId(item.getId()).getName()
						+ ": currently costs "
						+ shop.getCostValue(player, item)
						+ " "
						+ ItemDefinition.forId(shop.getCurrency()).getName()
								.toLowerCase() + ".");
	}
	 ...
	 */
	public static void openShop(Player player, int shopId, int stockType) {
		...
		if (shop.getMainStockId() != -1) {
			player.getActionSender().sendRunScript(25,
					new Object[] { shop.getMainStockId(), 93 },
					Constants.MAIN_STOCK_TYPE_STRING);
		} else {
			player.getActionSender().sendRunScript(25,
					new Object[] { 860, 93 }, Constants.MAIN_STOCK_TYPE_STRING); // We'll
																					// just
																					// send
																					// 860
																					// to
		...
		player.getActionSender().sendRunScript(150, Constants.BUY_PARAMETERS,
				Constants.MAIN_STOCK_OPTIONS_STRING);

		player.getActionSender().sendRunScript(150, Constants.SELL_PARAMETERS,
				Constants.MAIN_STOCK_OPTIONS_STRING);

		player.getActionSender().sendAccessMask(
				Constants.SCRIPT_OPTIONS_EXAMINE, 0,
				PLAYER_INVENTORY_INTERFACE, 0, 28);

		player.getActionSender().sendString(SHOP_INVENTORY_INTERFACE, 22,
				shop.getShopName());
		...
		case 1: // Main stock
			player.getActionSender()
					...
					// Player stock contents
					.sendInterfaceConfig(620, 25, true)
					// Unknown
					.sendInterfaceConfig(620, 26, false)
					// The player stock tab highlighted
					.sendInterfaceConfig(620, 27, true)
					// The player stock tab highlighted
					.sendInterfaceConfig(620, 29, false)
					// The main stock tab unhighlighted
					.sendInterfaceConfig(620, 34,
							shop.getShopType() != ShopType.GENERAL_STORE)
					.sendAccessMask(Constants.SCRIPT_OPTIONS_EXAMINE,
							SHOP_MAIN_STOCK, SHOP_INVENTORY_INTERFACE, 0, 40);
			player.getInterfaceState().addListener(shop.getMainStock(),
					new InterfaceContainerListener(player, -1, 2, 91));
			break;
		case 2: // Player stock
			player.getActionSender()
					.sendInterfaceConfig(620, 23, true)
					// Main stock contents
					.sendInterfaceConfig(620, 24, false)
					// Player stock contents
					.sendInterfaceConfig(620, 25, false)
					// Unknown
					.sendInterfaceConfig(620, 26, true)
					// The player stock tab highlighted
					.sendInterfaceConfig(620, 27, false)
					// The player stock tab highlighted
					.sendInterfaceConfig(620, 29, true)
					// The main stock tab unhighlighted
					.sendInterfaceConfig(620, 34,
							shop.getShopType() != ShopType.GENERAL_STORE)
					.sendAccessMask(Constants.SCRIPT_OPTIONS_EXAMINE,
							SHOP_PLAYER_STOCK, SHOP_INVENTORY_INTERFACE, 0, 40);
			player.getInterfaceState().addListener(shop.getPlayerStock(),
					new InterfaceContainerListener(player, -1, 2, 91));
			break;
		}
	}
	...
	public static void sellItem(Player player, int slot, int id, int amount) {
		...
		if (shop.getShopType() == ShopType.SPECIALIST_STORE_BUY) {
			if (shop.getMainStock().contains(item.getId())
					|| shop.getDefaultStock().contains(item.getId())) {
				canSell = true;
			}
		}
		...
		if (canSell) {
			Shop.openShop(player, player.getInterfaceState().getOpenShop(), 2); // Forces
			// open
			// the
			// player
			// stock
			int shopSlot = shop.getPlayerStock().contains(item.getId()) ? shop
					.getPlayerStock().getSlotById(item.getId()) : shop
					.getPlayerStock().freeSlot();
			if (shopSlot == -1) {
				player.getActionSender().sendMessage(
						"This shop is currently full.");
			} else {
				if (shop.getPlayerStock().get(shopSlot) != null) {
					if (shop.getPlayerStock().get(shopSlot).getCount() + amount < 1
							|| shop.getPlayerStock().get(shopSlot).getCount()
				...
				long totalPrice = (totalAmount * totalValue);
				if (totalPrice > Integer.MAX_VALUE) {
					amount = (Integer.MAX_VALUE / shop.getSellValue(player,
							item)) - 1;
				}
				Item reward = new Item(shop.getCurrency(), amount
						* shop.getSellValue(player, item));
				/*
				 * We make a temporary inventory container, why? This is because
				 * if we have 1 AGS, and 27 whips, we would have no free slot
				 * for the coins. Now on most servers, if you tried to sell the
				 * AGS, it would say "not enough inventory space", which is
				 * true, but wrong. On RS, it removes the AGS first, THEN adds
				 * the coins. However, we also want to keep our checks for
				 * inventory space in there, so we do it on a temporary
				 * inventory first.
				 */
				Container temporaryInventory = new Container(Type.STANDARD,
						Inventory.SIZE);
	...
	public static void valueItem(Player player, int slot, int id) {
		...
		player.getActionSender().sendMessage(
				message ? ItemDefinition.forId(item.getId()).getName()
						+ ": shop will buy for "
						+ finalValue
						+ " "
						+ ItemDefinition.forId(shop.getCurrency()).getName()
	 ...
	 *            The shop's currency.
	 */
	public Shop(String name, int mainStockId, ShopType shopType, int currency,
			Item[] mainItems, Item[] playerItems) {
		this.name = name;
	...
	public int getCostValue(Player player, Item item) {
		...
		case 1: // Main stock
			switch (getShopType()) {
			case GENERAL_STORE:
				return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.8 < 1 ? 1
						: ItemDefinition.forId(item.getId()).getValue() * 0.8);
		...
		case 2: // Player stock
			switch (getShopType()) {
			case GENERAL_STORE:
				return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.6 < 1 ? 1
						: ItemDefinition.forId(item.getId()).getValue() * 0.6);
			case SPECIALIST_STORE_BUY:
			case SPECIALIST_STORE_NO_BUY:
				return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.8 < 1 ? 1
						: ItemDefinition.forId(item.getId()).getValue() * 0.8);
	...
	public int getSellValue(Player player, Item item) {
		switch (getShopType()) {
		case GENERAL_STORE:
			return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.4 < 1 ? 1
					: ItemDefinition.forId(item.getId()).getValue() * 0.4);
		case SPECIALIST_STORE_BUY:
		case SPECIALIST_STORE_NO_BUY:
			return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.6 < 1 ? 1
					: ItemDefinition.forId(item.getId()).getValue() * 0.6);
