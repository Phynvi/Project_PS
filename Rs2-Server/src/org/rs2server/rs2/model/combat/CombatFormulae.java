...
public final class CombatFormulae {
	 ...
	 */
	public static final double DEFENCE_MODIFIER = 1;// 0.895;

	/**
	 * Calculates a mob's melee max hit.
	 */
	public static int calculateEffectiveAccuracy(Mob mob, boolean special,
			int skill, double bonus) {
		...
		// salve amulet(e) = 1.2
		int skillLevel = mob.getSkills().getLevel(skill);
		int combatStyleBonus = 0;

		if (skill == Skills.ATTACK) {
			// melee attack prayer modifiers
			if (mob.getCombatState().getPrayer(Prayers.CLARITY_OF_THOUGHT)) {
				prayerMultiplier *= 1.05;
			} else if (mob.getCombatState()
					.getPrayer(Prayers.IMPROVED_REFLEXES)) {
				prayerMultiplier *= 1.10;
			} else if (mob.getCombatState().getPrayer(
			...
			if (mob.getCombatState().getPrayer(Prayers.SHARP_EYE)) {
				prayerMultiplier *= 1.05;
			} else if (mob.getCombatState().getPrayer(Prayers.HAWK_EYE)) {
				prayerMultiplier *= 1.10;
			} else if (mob.getCombatState().getPrayer(Prayers.EAGLE_EYE)) {
				prayerMultiplier *= 1.15;
			}
		} else if (skill == Skills.MAGIC) {
			// magic attack prayer modifiers
			if (mob.getCombatState().getPrayer(Prayers.MYSTIC_WILL)) {
				prayerMultiplier *= 1.05;
			} else if (mob.getCombatState().getPrayer(Prayers.MYSTIC_LORE)) {
				prayerMultiplier *= 1.10;
			} else if (mob.getCombatState().getPrayer(Prayers.MYSTIC_MIGHT)) {
				prayerMultiplier *= 1.15;
			}
		} else if (skill == Skills.DEFENCE) {
			if (mob.getCombatState().getPrayer(Prayers.THICK_SKIN)) {
				prayerMultiplier *= 1.05;
			} else if (mob.getCombatState().getPrayer(Prayers.ROCK_SKIN)) {
				prayerMultiplier *= 1.10;
			} else if (mob.getCombatState().getPrayer(Prayers.STEEL_SKIN)) {
				prayerMultiplier *= 1.15;
			} else if (mob.getCombatState().getPrayer(Prayers.CHIVALRY)) {
				prayerMultiplier *= 1.20;
			} else if (mob.getCombatState().getPrayer(Prayers.PIETY)) {
		...
		if (skillLevel <= 10) {
			otherBonusMultiplier = 50;
		}

		int accuracy = (int) ((skillLevel * prayerMultiplier * otherBonusMultiplier) + combatStyleBonus);
		double baseAccuracy = 1.3 + (accuracy / 10) + (bonus / 80)
				+ ((accuracy * bonus) / 640);
		...
		int effectiveAccuracy = (int) (baseAccuracy * specialMultiplier);
	...
	public static int calculateMeleeMaxHit(Mob mob, boolean special) {
		...
		int attackLevel = mob.getSkills().getLevel(Skills.ATTACK);
		int combatStyleBonus = 0;
	...
	public static int calculateRangeMaxHit(Mob mob, boolean special) {
		...
		if (bow == BowType.CRYSTAL_BOW) {
			 ...
			 * strength bonus.
			 */
			rangedStrength = mob.getEquipment().get(Equipment.SLOT_WEAPON)
					.getEquipmentDefinition().getBonus(12);
		}
		...
		if (special) {
			if (mob.getEquipment().get(Equipment.SLOT_ARROWS) != null) {
				...
				case 892:
					if (mob.getEquipment().get(Equipment.SLOT_WEAPON) != null
							&& mob.getEquipment().get(Equipment.SLOT_WEAPON)
									.getId() == 11235) {
	...
	public static boolean fullAhrim(Mob mob) {
		return mob.getEquipment() != null && mob.getEquipment().contains(4708)
	...
	public static boolean fullDharok(Mob mob) {
		return mob.getEquipment() != null && mob.getEquipment().contains(4716)
	...
	public static boolean fullGuthan(Mob mob) {
		return mob.getEquipment() != null && mob.getEquipment().contains(4724)
	...
	public static boolean fullKaril(Mob mob) {
		return mob.getEquipment() != null && mob.getEquipment().contains(4732)
	...
	public static boolean fullTorag(Mob mob) {
		return mob.getEquipment() != null && mob.getEquipment().contains(4745)
	...
	public static boolean fullVerac(Mob mob) {
		return mob.getEquipment() != null && mob.getEquipment().contains(4753)
	...
	public static boolean fullVoidMage(Mob mob) {
		return mob.getEquipment() != null && mob.getEquipment().contains(8839)
	...
	public static CombatAction getActiveCombatAction(Mob mob) {
		...
		if (mob.getCombatState().getQueuedSpell() != null
				|| (mob.getAutocastSpell() != null && (mob.getCombatState()
						.getCombatStyle() == CombatStyle.AUTOCAST || mob
						.getCombatState().getCombatStyle() == CombatStyle.DEFENSIVE_AUTOCAST))) {
		...
		if (weapon != null) {
			...
			if (weaponEquipDef.getBowType() != null
					|| weaponEquipDef.getRangeWeaponType() != null) {
	...
	public static int getCombatCooldownDelay(Mob mob) {
		...
		if (getActiveCombatAction(mob) == RangeCombatAction.getAction()) {
			if (mob.getCombatState().getCombatStyle() != CombatStyle.AGGRESSIVE_1) {
				/**
				 * If we are ranging and are not on rapid, combat speed is
				 * increased by 1 cycle
				 */
				extra = 1;
			}
		}
		return (mob.getEquipment() != null && mob.getEquipment().get(3) != null) ? mob
				.getEquipment().get(3).getEquipmentDefinition().getSpeed()
				+ extra
				: 4;
	}
