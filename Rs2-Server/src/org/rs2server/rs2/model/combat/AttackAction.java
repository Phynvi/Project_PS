...
public class AttackAction extends Action {
	...
	private void attack(Mob target) {
		...
		if (action == MagicCombatAction.getAction()
				&& mob.getCombatState().getQueuedSpell() == null
	...
	public void execute() {
		...
		if (target == null || target.isDestroyed()
				|| target.getCombatState().isDead()
				|| mode != InteractionMode.ATTACK) {
		...
		if (mob.isNPC()) {
			if (!mob.getLocation().isWithinDistance(mob.getWidth(),
					mob.getHeight(), target.getLocation(), target.getWidth(),
					target.getHeight(),
					mob.getActiveCombatAction().distance(mob, target) + 5)) {
				mob.resetInteractingEntity();
				this.stop();
				return;
			}
		...
		int requiredDistance = mob.getActiveCombatAction()
				.distance(mob, target);
		...
		if (distance <= requiredDistance) { // only reset walking queue when
											// they are exactly in distance, and
											// not still moving
		 ...
		 */
		if (CombatFormulae.getActiveCombatAction(mob) == MagicCombatAction
				.getAction()) {
			if (mob.getCombatState().getSpellDelay() > 0
					|| mob.getCombatState().getAttackDelay() > 2) {
				return;
			}
		} else {
		...
		if (target != null && mode == InteractionMode.ATTACK) {
			attack(target);
			if (CombatFormulae.getActiveCombatAction(mob) != MagicCombatAction
					.getAction() && mob.getCombatState().getAttackDelay() == 0) {
				mob.getCombatState().setAttackDelay(
						mob.getCombatCooldownDelay());
			}
		} else {
