...
public class MeleeCombatAction extends AbstractCombatAction {
	...
	public void addExperience(Mob attacker, int damage) {
		...
		for (int i = 0; i < attacker.getCombatState().getCombatStyle()
				.getSkills().length; i++) {
			attacker.getSkills().addExperience(
					attacker.getCombatState().getCombatStyle().getSkill(i),
					(attacker.getCombatState().getCombatStyle()
							.getExperience(i) * damage)
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		final int maxHit = CombatFormulae.calculateMeleeMaxHit(attacker,
				special);
		...
		if (!special) {
			...
			if (attacker.getEquipment().get(Equipment.SLOT_WEAPON) != null
					&& attacker.getEquipment().get(Equipment.SLOT_WEAPON)
							.getEquipmentDefinition() != null) {
				int length = attacker.getEquipment().get(Equipment.SLOT_WEAPON)
						.getEquipmentDefinition().getAnimation().getAttacks().length;
				if (attackAnimationIndex >= length) {
					attackAnimationIndex = length - 1;
				}
			}
			attacker.playAnimation((attacker.getEquipment().get(
					Equipment.SLOT_WEAPON) != null && attacker.getEquipment()
					.get(Equipment.SLOT_WEAPON).getEquipmentDefinition() != null) ? attacker
		...
		if (victim.getCombatState().getPoisonDamage() < 1
			...
			if (attacker.getEquipment().get(Equipment.SLOT_WEAPON) != null
					&& attacker.getEquipment().get(Equipment.SLOT_WEAPON)
							.getEquipmentDefinition() != null) {
				if (attacker.getEquipment().get(Equipment.SLOT_WEAPON)
						.getEquipmentDefinition().getPoisonType() != PoisonType.NONE) {
					victim.getCombatState().setPoisonDamage(
							attacker.getEquipment().get(Equipment.SLOT_WEAPON)
									.getEquipmentDefinition().getPoisonType()
		...
		if (special) {
			final Item weapon = attacker.getEquipment().get(
					Equipment.SLOT_WEAPON);
			if (weapon != null && weapon.getEquipmentDefinition() != null) {
				World.getWorld().submit(new Tickable(1) {
					@Override
					public void execute() {
						for (int i = 1; i < weapon.getEquipmentDefinition()
								.getSpecialHits(); i++) {
							int dmg = damage(maxHit, attacker, victim, attacker
									.getCombatState().getAttackType(),
									Skills.ATTACK, Prayers.PROTECT_FROM_MELEE,
									true, false);
							int specialHit = random.nextInt(dmg < 1 ? 1
									: dmg + 1);
							if (specialHit > victim.getSkills().getLevel(
									Skills.HITPOINTS)) {
								specialHit = victim.getSkills().getLevel(
										Skills.HITPOINTS);
							}
