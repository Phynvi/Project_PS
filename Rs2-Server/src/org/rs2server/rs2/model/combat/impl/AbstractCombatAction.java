...
public abstract class AbstractCombatAction implements CombatAction {
	...
	public void addExperience(Mob attacker, int damage) {
		attacker.getSkills().addExperience(Skills.HITPOINTS,
				(damage * 1.3) * Constants.getExpModifier());
	}
	...
	public boolean canHit(Mob attacker, Mob victim, boolean messages,
		...
		if (!BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(),
				"MultiCombat")
				|| !BoundaryManager.isWithinBoundaryNoZ(victim.getLocation(),
						"MultiCombat")) {
			if (attacker.getCombatState().getLastHitTimer() > (System
					.currentTimeMillis() + 4000)) { // 10 cycles for tagging
													// timer
			...
			if (victim.getCombatState().getLastHitTimer() > (System
					.currentTimeMillis() + 4000)) { // 10 cycles for tagging
													// timer
	...
	public boolean canSpecial(final Mob attacker, final Mob victim) {
		...
		if (weapon != null && weapon.getEquipmentDefinition() != null) {
			if (victim != null
					&& !victim.getCombatState().isDead()
					&& attacker.getActiveCombatAction().canHit(attacker,
							victim, true, false)
					&& attacker.getLocation().isWithinDistance(
							attacker,
							victim,
							attacker.getActiveCombatAction().distance(attacker,
									victim))) {
				if (attacker.getCombatState().getSpecialEnergy() >= weapon
						.getEquipmentDefinition().getSpecialConsumption()) {
					if (attacker.getCombatState().getAttackDelay() == 0) {
						switch (weapon.getId()) {
						case 861:
						case 11235:
							if (attacker.getEquipment()
									.get(Equipment.SLOT_ARROWS).getCount() < 2) {
								attacker.getActionSender()
										.sendMessage(
												"You need atleast 2 arrows to perform this special.");
						...
						case 4153:
							if (!attacker.canAnimate()
									|| attacker.getActiveCombatAction() != MeleeCombatAction
											.getAction()
									|| !attacker.getActiveCombatAction()
											.canHit(attacker, victim, true,
													false)
									|| !attacker
											.getLocation()
											.isWithinDistance(
	...
	/*
	 * @Override public int damage(int maxHit, Mob attacker, Mob victim,
	 * AttackType attackType, int skill, int prayer, boolean special, boolean
	 * ignorePrayers) { boolean veracEffect = false; if (skill == Skills.ATTACK)
	 * { if (CombatFormulae.fullVerac(attacker)) { if (random.nextInt(8) == 3) {
	 * veracEffect = true; } } }
	 * 
	 * double attackBonus =
	 * attacker.getCombatState().getBonus(attackType.getId()) == 0 ? 1 :
	 * attacker.getCombatState().getBonus(attackType.getId()); if (attackBonus <
	 * 1) { attackBonus = 1; } double attackCalc = attackBonus *
	 * attacker.getSkills().getLevel(skill); // +1 as its exclusive
	 * 
	 * 
	 * // Prayer calculations.
	 * 
	 * if (skill == Skills.ATTACK) { //melee attack prayer modifiers if
	 * (attacker.getCombatState().getPrayer(Prayers.CLARITY_OF_THOUGHT)) {
	 * attackCalc *= 1.05; } else if
	 * (attacker.getCombatState().getPrayer(Prayers.IMPROVED_REFLEXES)) {
	 * attackCalc *= 1.10; } else if
	 * (attacker.getCombatState().getPrayer(Prayers.INCREDIBLE_REFLEXES)) {
	 * attackCalc *= 1.15; } else if
	 * (attacker.getCombatState().getPrayer(Prayers.CHIVALRY)) { attackCalc *=
	 * 1.15; } else if (attacker.getCombatState().getPrayer(Prayers.PIETY)) {
	 * attackCalc *= 1.20; } } else if (skill == Skills.RANGE) { //range attack
	 * prayer modifiers if
	 * (attacker.getCombatState().getPrayer(Prayers.SHARP_EYE)) { attackCalc *=
	 * 1.05; } else if (attacker.getCombatState().getPrayer(Prayers.HAWK_EYE)) {
	 * attackCalc *= 1.10; } else if
	 * (attacker.getCombatState().getPrayer(Prayers.EAGLE_EYE)) { attackCalc *=
	 * 1.15; } } else if (skill == Skills.MAGIC) { //magic attack prayer
	 * modifiers if (attacker.getCombatState().getPrayer(Prayers.MYSTIC_WILL)) {
	 * attackCalc *= 1.05; } else if
	 * (attacker.getCombatState().getPrayer(Prayers.MYSTIC_LORE)) { attackCalc
	 * *= 1.10; } else if
	 * (attacker.getCombatState().getPrayer(Prayers.MYSTIC_MIGHT)) { attackCalc
	 * *= 1.15; } }
	 * 
	 * 
	 ...
	 * attackCalc += 3; } else if (attacker.getCombatState().getCombatStyle() ==
	 * CombatStyle.CONTROLLED_1 || attacker.getCombatState().getCombatStyle() ==
	 * CombatStyle.CONTROLLED_2 || attacker.getCombatState().getCombatStyle() ==
	 * CombatStyle.CONTROLLED_3) { attackCalc += 1; }
	 * 
	 * double defenceBonus = victim.getCombatState().getBonus(attackType.getId()
	 * + 5) == 0 ? 1 : victim.getCombatState().getBonus(attackType.getId() + 5);
	 * double defenceCalc = defenceBonus *
	 * victim.getSkills().getLevel(Skills.DEFENCE); // +1 as its exclusive
	 * 
	 * //Prayer calcs if (victim.getCombatState().getPrayer(Prayers.THICK_SKIN))
	 * { defenceCalc *= 1.05; } else if
	 * (victim.getCombatState().getPrayer(Prayers.ROCK_SKIN)) { defenceCalc *=
	 * 1.10; } else if (victim.getCombatState().getPrayer(Prayers.STEEL_SKIN)) {
	 * defenceCalc *= 1.15; } else if
	 * (victim.getCombatState().getPrayer(Prayers.CHIVALRY)) { defenceCalc *=
	 * 1.20; } else if (victim.getCombatState().getPrayer(Prayers.PIETY)) {
	 * defenceCalc *= 1.25; }
	 * 
	 * 
	 * // As with the melee/range max hit calcs, combat style bonuses are added
	 ...
	 * defenceCalc += 3; } else if (victim.getCombatState().getCombatStyle() ==
	 * CombatStyle.CONTROLLED_1 || victim.getCombatState().getCombatStyle() ==
	 * CombatStyle.CONTROLLED_2 || victim.getCombatState().getCombatStyle() ==
	 * CombatStyle.CONTROLLED_3) { defenceCalc += 1; }
	 * 
	 * if (veracEffect) { defenceCalc = 0; }
	 * 
	 * 
	 * // The chance to succeed out of 1.0.
	 * 
	 * double hitSucceed = CombatFormulae.DEFENCE_MODIFIER * (attackCalc /
	 * defenceCalc); if (hitSucceed > 1.0) { hitSucceed = 1; } if (hitSucceed <
	 * random.nextDouble()) { return 0; } else { /** Protection prayers. Note:
	 * If an NPC is hitting on a protection prayer, it is 100% blocked, where as
	 * if a player is hitting on a protection prayer, their damage is simply
	 * reduced by 40%. Also, if the attacker has the Verac effect active, it
	 * will ignore the opponent's protection prayers.
	 */
	/*
	 * int protectionPrayer = (int) ((victim.getCombatState().getPrayer(prayer)
	 * && !veracEffect) ? attacker.getProtectionPrayerModifier() : 1); if
	 * (ignorePrayers) { protectionPrayer = 1; } int hit = maxHit *
	 * protectionPrayer; // +1 as its exclusive return hit; } }
	 */
	@Override
	public int damage(int maxHit, Mob attacker, Mob victim,
		...
		double effectiveAttack = CombatFormulae.calculateEffectiveAccuracy(
				attacker, special, skill, attackBonus); // +1 as its exclusive

		double defenceBonus = victim.getCombatState().getBonus(
				attackType.getId() + 5) == 0 ? 1 : victim.getCombatState()
				.getBonus(attackType.getId() + 5);
		double effectiveDefence = CombatFormulae.calculateEffectiveAccuracy(
				victim, false, Skills.DEFENCE, defenceBonus);

		if (veracEffect) {
			effectiveDefence = 0;
		}

		double attackerRoll = effectiveAttack * random.nextDouble();
		double defenderRoll = effectiveDefence * random.nextDouble();
		if (attackerRoll == 0) {
			attackerRoll = 1;
		}
		if (defenderRoll == 0) {
		...
		/**
		 * System.out.println(attackerRoll); System.out.println(defenderRoll);
		 * System.out.println(effectiveAttack);
		 * System.out.println(effectiveDefence);
		 * System.out.println(attacker.isNPC());
		...
		// double accuracy;
		/*
		 * if (attackerRoll < defenderRoll) { accuracy = (attackerRoll - 1) / (2
		 * * defenderRoll); } else if (attackerRoll > defenderRoll) { accuracy =
		 * 1 - (defenderRoll + 1) / (2 * attackerRoll); } else { accuracy = 1 -
		 * (defenderRoll + 1) / (2 * attackerRoll); }
		 */
		if (defenderRoll > attackerRoll) {
			 ...
			 */
			int protectionPrayer = (int) ((victim.getCombatState().getPrayer(
					prayer) && !veracEffect) ? attacker
					.getProtectionPrayerModifier() : 1);
			if (ignorePrayers) {
				protectionPrayer = 1;
			}
			int hit = maxHit * protectionPrayer; // +1 as its exclusive
			return hit;
		}
	}
	...
	public void defend(Mob attacker, Mob victim, boolean blockAnimation) {
		if (victim.isAutoRetaliating()) {
			if (victim.getCombatState().getAttackEvent() == null
					|| victim.getInteractingEntity() != attacker) {
	...
	public void hit(final Mob attacker, Mob victim) {
		try {
			if (attacker.getAttribute("casting") == null) {
				Sound.playAttackSound(attacker, victim);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		/**
		 * This is to prevent immediate teaming, EG: someone walking into
		 * Edgeville(1v1) and 2 people casting spells at the same time, usually
		 * it wouldn't set the timer till the damage had been inflicted
		 */
	...
	public void recoil(Mob attacker, Mob victim, int damage) {
		if (victim.getEquipment().get(Equipment.SLOT_RING) != null
			...
			if (damage > 0) {
				...
				victim.getCombatState().setRingOfRecoil(
						victim.getCombatState().getRingOfRecoil() - recoil);
				attacker.inflictDamage(
						new Hit(recoil, HitPriority.LOW_PRIORITY), victim);
	...
	public void special(Mob attacker, final Item item) {
		if (attacker.getInteractingEntity() == null
				|| attacker.getInteractionMode() != InteractionMode.ATTACK) {
			return;
		}
		switch (item.getId()) {
		case 4153:
			...
			Hit hit = new Hit(random.nextInt(attacker.getActiveCombatAction()
					.damage(damage, attacker, attacker.getInteractingEntity(),
							AttackType.CRUSH, Skills.ATTACK,
							Prayers.PROTECT_FROM_MELEE, true, false) + 1));
			if (hit.getDamage() > attacker.getInteractingEntity().getSkills()
	...
	public void special(final Mob attacker, final Mob victim, int damage) {
		try {
			Sound.playSpecialAttackSound(attacker, victim);
		} catch (Exception e) {
			e.printStackTrace();
		}
		Item weapon = attacker.getEquipment().get(Equipment.SLOT_WEAPON);
		BowType bow = weapon.getEquipmentDefinition().getBowType();
		if (bow == BowType.BRONZE_CBOW || bow == BowType.IRON_CBOW
				|| bow == BowType.STEEL_CBOW || bow == BowType.MITH_CBOW
				|| bow == BowType.ADAMANT_CBOW || bow == BowType.RUNE_CBOW) {
			weapon = attacker.getEquipment().get(Equipment.SLOT_ARROWS);
		}
		attacker.getCombatState().setSpecial(true);
		switch (weapon.getId()) {
		case 4151:
			attacker.playAnimation(weapon.getEquipmentDefinition()
					.getAnimation().getAttack(0));
		...
		case 11694:
			...
			break;
		case 11696:
			int[] skills = new int[] { Skills.DEFENCE, Skills.STRENGTH,
					Skills.PRAYER, Skills.ATTACK, Skills.MAGIC, Skills.RANGE };
			int newDmg = damage / 10;
			for (int i = 0; i < skills.length; i++) {
				if (newDmg > 0) {
					if (victim.getSkills().getLevel(skills[i]) > 0) {
						int before = victim.getSkills().getLevel(skills[i]);
						victim.getSkills().decreaseLevelToZero(skills[i],
								newDmg);
						int after = before
								- victim.getSkills().getLevel(skills[i]);
						newDmg -= after;
					}
				} else {
					break;
		...
		case 11698:
			...
			attacker.getSkills().increaseLevelToMaximum(Skills.HITPOINTS,
					hitpointsHeal);
			attacker.getSkills().increaseLevelToMaximum(Skills.PRAYER,
					prayerHeal);
			break;
		case 11700:
		...
		case 11235:
			...
			attacker.playProjectile(Projectile.create(
					...
					attacker.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212 ? 1099
							: 1101, showDelay, 50, clientSpeed - 10, 41, 31,
					victim.getProjectileLockonIndex(), 3, 36));
			attacker.playProjectile(Projectile.create(
					attacker.getLocation(),
					victim.getCentreLocation(),
					attacker.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212 ? 1099
							: 1101, showDelay, 50, clientSpeed + 10, 46, 36,
					victim.getProjectileLockonIndex(), slope + 6, 36));
			victim.playGraphics(Graphic
					.create(attacker.getEquipment().get(Equipment.SLOT_ARROWS)
							.getId() == 11212 ? 1100 : 1103, clientSpeed, 100));
			break;
		case 861:
			int distance = attacker.getLocation().distanceToEntity(attacker,
					victim);
			...
			attacker.playProjectile(Projectile.create(attacker.getLocation(),
					victim.getCentreLocation(), 249, 30, 50,
					40 + (distance * 5), 43, 35,
					victim.getProjectileLockonIndex(), 10, 36));
			attacker.playProjectile(Projectile.create(attacker.getLocation(),
					victim.getCentreLocation(), 249, 60, 50,
					65 + (distance * 5), 43, 35,
					victim.getProjectileLockonIndex(), 10, 36));
			break;
		case 805:
			...
			if (BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(),
					"MultiCombat")) {
				for (Mob mob : World.getWorld().getRegionManager()
						.getLocalMobs(victim)) {
					boolean canContinue = true;
					for (int i = 0; i < closeMobs.length; i++) {
						if (canContinue) {
							int newDist = attacker.getLocation()
									.distanceToEntity(attacker, mob);
							if (newDist <= distances[i]) {
								if (mob != attacker
										&& mob != victim
										&& mob.getLocation().isWithinDistance(
												attacker, mob, 5)
										&& attacker.getActiveCombatAction()
												.canHit(attacker, mob, false,
														false)
			...
			int count = 3;
			final int maxHit = CombatFormulae.calculateRangeMaxHit(attacker,
					true);
			final int newDamage = damage(maxHit, attacker, victim, attacker
			...
			if (BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(),
				...
				for (final Mob mob : closeMobs) {
					...
					lastMob.playProjectile(Projectile.create(
							lastMob.getLocation(), mob.getCentreLocation(),
							258, lastDelay, 50, lastSpeed, 43, 35,
							mob.getProjectileLockonIndex(), 13, 48));
					World.getWorld().submit(new Tickable(count) {

						@Override
						public void execute() {
							int hit = random.nextInt(newDamage < 1 ? 1
									: newDamage + 1); // +1 as its exclusive
							mob.inflictDamage(new Hit(hit), attacker);
							smite(attacker, mob, hit);
							recoil(attacker, mob, hit);
							victim.getActiveCombatAction().defend(attacker,
									mob, true);
							this.stop();
						}
					});
			...
			attacker.playProjectile(Projectile.create(attacker.getLocation(),
					victim.getCentreLocation(), 258, 45, 50, 55, 43, 35,
					victim.getProjectileLockonIndex(), 13, 48));
			attacker.getCombatState().setSpecial(false);
			break;
		case 9236:
		...
		case 9240:
			...
			if (victim.isPlayer()) {
				...
				victim.getSkills().decreaseLevelToZero(Skills.PRAYER,
						prayerReduction);
				int addition = before
						- victim.getSkills().getLevel(Skills.PRAYER);
				attacker.getSkills().increaseLevelToMaximum(Skills.PRAYER,
						addition);
			}
			break;
		...
		attacker.getCombatState().decreaseSpecial(
				weapon.getEquipmentDefinition().getSpecialConsumption());
		attacker.getCombatState().inverseSpecial();
	}

	@Override
	public void vengeance(final Mob attacker, final Mob victim,
			final int damage, int delay) {
		...
		World.getWorld().submit(new Tickable(delay) {
			...
			public void execute() {
				...
				attacker.inflictDamage(new Hit(hit > attacker.getSkills()
						.getLevel(Skills.HITPOINTS) ? attacker.getSkills()
						.getLevel(Skills.HITPOINTS) : hit), victim);
				this.stop();
			}
		});
	}
