...
public class ItemSpawn {
	 ...
	 * The logger instance.
	 */
	private static final Logger logger = Logger.getLogger(ItemSpawn.class
			.getName());
	...
	public static void init() throws IOException {
		...
		// spawns.add(new ItemSpawn(new Item(223, 1), Location.create(2832,
		// 9585, 0)));
		// XMLController.writeXML(spawns, file);
		if (file.exists()) {
			spawns = XMLController.readXML(file);
			for (ItemSpawn spawn : spawns) {
				GroundItem groundItem = new GroundItem("", spawn.getItem(),
						spawn.getLocation());
