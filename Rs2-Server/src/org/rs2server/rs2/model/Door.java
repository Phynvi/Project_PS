...
public class Door {
	...
	public boolean isDoubleDoor() {
		return secondLocation != null && secondLocation != object.getLocation()
				&& secondLocation != replacement.getLocation();
	}
	...
	public void open(boolean openSecond) {
		if (!isReplaced()) {
			boolean locationsEqual = getObject().getLocation().equals(
					getReplacement().getLocation());
			World.getWorld().unregister(getObject(), false);
			World.getWorld().register(getReplacement());
			if (!locationsEqual) {
				removedGameObject = new GameObject(getObject().getLocation(),
						-1, getObject().getType(),
						getObject().getDirection() + 1, false);
		...
		if (openSecond && secondLocation != null) {
			Door secondDoor = replacement.getRegion().doorForLocation(
					secondLocation, secondId);
