...
public class Herblore extends ProductionAction {
	...
	public static enum Herb {
		...
		GUAM(199, 249, 1, 2.5), MARRENTILL(201, 251, 5, 3.8), TARROMIN(203,
				253, 11, 5), HARRALANDER(205, 255, 20, 6.3), RANARR(207, 257,
				25, 7.5), IRIT(209, 259, 40, 8.8), AVANTOE(211, 261, 48, 10), KWUARM(
				213, 263, 54, 11.3), CADANTINE(215, 265, 65, 12.5), DWARF_WEED(
				217, 267, 70, 13.8), TORSTOL(219, 269, 75, 15);
		/**
		 * The id of the herb
		 */
		private int id;
	...
	public static enum SecondaryIngredient {
		...
		private SecondaryIngredient(int index, int id, Item requiredItem,
				int reward, int level, int exp) {
			this.index = index;
			this.id = id;
	...
	public Herblore(Mob mob, int productionCount,
	...
	public Item[] getConsumedItems() {
		...
		case PRIMARY_INGREDIENT:
			return new Item[] { new Item(primaryIngredient.getId()),
					new Item(227) };
		case SECONDARY_INGREDIENT:
			return new Item[] { new Item(secondaryIngredient.getId()),
					secondaryIngredient.getRequiredItem() };
		}
		return null;
	}
	...
	public String getInsufficentLevelMessage() {
		return "You need a " + Skills.SKILL_NAME[getSkill()] + " level of "
				+ getRequiredLevel() + " to combine these ingredients.";
	}
	...
	@Override
	public int getRequiredLevel() {
		return herbloreType == HerbloreType.PRIMARY_INGREDIENT ? primaryIngredient
				.getRequiredLevel() : secondaryIngredient.getRequiredLevel();
	}
	...
	public String getSuccessfulProductionMessage() {
		...
		case PRIMARY_INGREDIENT:
			return "You put the "
					+ ItemDefinition.forId(primaryIngredient.getId()).getName()
							.replaceAll(" clean", "").replaceAll(" leaf", "")
							.toLowerCase() + " into the vial of water.";
		case SECONDARY_INGREDIENT:
			return "You mix the "
					+ ItemDefinition.forId(secondaryIngredient.getId())
							.getName().toLowerCase() + " into your potion.";
		}
		return "";
	}
}
