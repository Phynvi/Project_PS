...
public class Slayer {
	...
	private static final Slayer singleton = new Slayer();
	...
	/*
	 * public void handleSlayerGem(Player player) { //4155 is gem if
	 * (player.getSlayerTask() != null && player.getSlayerTask().hasTask()) {
	 * String name = player.getSlayerTask().getName(); int master =
	 * player.getSlayerTask().getSlayerMaster(); int count =
	 * player.getSlayerTask().getCount(); String message = "There are " + count
	 * + " " + name + " remaining.";
	 * player.getActionSender().sendDialogue(NPCDefinition
	 * .forId(master).getName(), DialogueType.NPC, master,
	 * FacialAnimation.DEFAULT, message + ""); } else {
	 * player.getActionSender().
	 * sendMessage("I should get a slayer task first."); } //josh }
	 */
	...
	public static String[] TURAEL_NAMES = { "Banshees", /*
														 * "Bats", "Birds",
														 */
	"Bears", /*
			 * "Cave bugs", "Cave slimes",
			 */"Cows", "Crawling Hands",
	/*
	 * "Desert Lizards", "Dogs",
	 */"Dwarves", "Ghosts", "Goblins",
	/*
	 * "Icefiends",
	 */"Minotaurs", /*
					 * "Monkeys",
					 */"Scorpions", "Skeletons", "Spiders", "Wolves", "Zombies" };
	public static final int[][] TURAEL_MONSTERS = { BANSHEES,
	/*
	 * BATS, BIRDS,
	 */
	BEARS,
	/*
	 * CAVE_BUGS, CAVE_SLIMES,
	 */
	COWS, CRAWLING_HANDS,
	/*
	 * DESERT_LIZARDS, DOGS,
	 ...
	 * MONKEYS,
	 */
	SCORPIONS, SKELETONS, SPIDERS, WOLFES, ZOMBIES, };
	/**
	...
	private static final int[] GHOULS = { 1218, 3059 /*
													 * Ghoul champion
													 */
	...
	public static String[] MAZCHNA_NAMES = { "Banshees", /*
														 * "Bats",
														 */"Bears",
	/*
	 ...
	 */"Ghosts", "Hill Giants", /*
								 ...
								 */"Ice warriors",
	/*
	 * "Kalphites", "Mogres", "Pyrefiends", "Rock slugs",
	 */"Skeletons",
	/*
	 * "Vampires",
	 */"Wolfs", "Zombies" };
	public static final int[][] MAZCHNA_MONSTERS = { BANSHEES,
	/*
	 * BATS,
	 */
	BEARS,
	...
	ICE_WARRIORS,
	/*
	 * KALPHITES, MOGRES, PYREFIENDS, ROCK_SLUGS,
	 */
	SKELETONS,
	/*
	 * VAMPIRES,
	...
	private static final int[] LESSER_DEMONS = { 82, 3064/*
														 * Lesser demon champ
														 */, 4694, 4695, 4696,
			4697, 6101 };
	...
	"Banshees", /*
				 * "Basilisks", "Bloodvelds", "Brine rats", "Cockatrices",
				 * "Crocodiles", "Dust devis", "Earth warriors", "Elves",
				 * "Ghouls",
				 */
	"Green dragons",/* "Harpie bug swarms", */"Hill giants", // "Ice giants",
			"Ice warriors", "Infernal mages",/*
											 * "Jellys", "Jungle horrors",
											 * "Killerwatts",
											 */"Lesser demons", // "Mogres",
																// "Molanisks",
			"Moss giants", /*
							 * "Ogres", "Otherworldly beings", "Pyrefiends",
							 * "Sea snakes", "Shades", "Shadow warriors",
							 * "Turoths", "Vampires", "Werewolfs"
							 */
	};
	public static final int[][] VANNAKA_MONSTERS = { ABERRANT_SPECTRES,
	/* ANKOUS, */
	BANSHEES,
	/*
	 * BASILISKS, BLOODVELDS, BRINE_RATS, COCKATRICES, CROCODILES, DUST_DEVILS,
	 * EARTH_WARRIORS, ELVES, GHOULS,
	 */
	GREEN_DRAGONS,
			// HARPIE_BUG_SWARMS,
			HILL_GIANTS,
			// ICE_GIANTS,
			ICE_WARRIORS, INFERNAL_MAGES,
			// JELLYS,
			// JUNGLE_HORRORS,
			// KILLERWATTS,
			LESSER_DEMONS,
	...
	private static final int[] HELLHOUNDS = { 49, 1575/*
													 * Skeletal hellhound
													 */, 3586 };

	private static final int[] IRON_DRAGONS = { 1591 };
	private static final int[] NECHRYAELS = { 1613 };
	private static final int[] SCABRITES = {/*
											 * Start of Locust riders
											 */5251, 5252, 5255, 5256,/*
																	 * end of
																	 * them
																	 */1969,
			2001, 2009, 4500, 5250, 5253, 5254 }; // Might be needing alot of
	// ids.
	...
	public static final String[] DURADEL_NAMES = { "Aberrant spectres",
			"Abyssal demons",/*
							 * "Aquanites",
							 */"Black demons", "Black dragons", "Bloodvelds",
			"Dagannoths", "Dark beasts", "Dust devis", "Fire giants",
			"Gargoyles", "Goraks", "Greater demons", "Hellhound",
			"Iron dragon", "Kalphites",
			/*
			 * "Mithril dragons",
			 */"Nechryaels", "Scabarites", "Skeleton Wyvern", /*
															 * "Spiritual Mage",
															 */"Steel dragon",
			"Suqah", "Waterfiend", /*
									 * "Warped terrorbirds"
	...
	public static Slayer getSingleton() {
		return singleton;
	}

	public boolean canHit(Player c, int type) {
		switch (type) {
		/*
		 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~`~~~~~~~~~~~~~~`START OF SLAYER TOWER
		 * CREATURES~~~~~~~~~~~~~~~~~~~~~~~~~~~`
		 */
		/*
		 * Crawling Hands
		...
		case 1656:
		case 1657:
			if (c.getSkills().getLevel(18) < 0) {// default at 0 - no tasks at,
													// 5 is regular
				c.getActionSender().sendMessage(
						"You have no knowledge of how to wound this creature!");
				return false;
			}
			break;
		 ...
		 */
		case 1612:
			if (c.getSkills().getLevel(18) < 15) {
				// c.getActionSender().sendMessage("You have no knowledge of how to wound this creature!");
				return false;
			}
			if (c.getEquipment().get(Equipment.SLOT_HELM) == null) {
				// c.getActionSender().sendMessage("Maybe i'm forgetting something, I don't want to go deaf...");
				return false;
			}
			if (c.getSkills().getLevel(18) >= 1
					&& c.getEquipment().get(Equipment.SLOT_HELM).getId() != 4166) {
				// c.getActionSender().sendMessage("Maybe i'm forgetting something, I don't want to go deaf...");
				return false;
			}
			break;
		...
		case 1606:
		case 1607:
			if (c.getSkills().getLevel(18) < 60) {
				// c.getActionSender().sendMessage("You have no knowledge of how to wound this creature!");
				return false;
			}
			if (c.getSkills().getLevel(18) >= 60
					&& (c.getEquipment().get(Equipment.SLOT_HELM) == null || c
							.getEquipment().get(Equipment.SLOT_HELM).getId() != 4168)) {
				// c.getActionSender().sendMessage("These things smell, maybe I can do something about it...");
				return false;
			}
			break;
	 ...
	 */
	private int getChaeldarIndex(String previousName, int slayerLevel) {
	...
	private int getCount(int fromRange, int toRange) {
	 ...
	 * @return An array index used to get an NPC array and a Name array.
	 */
	private int getDuradelIndex(String previousName, int slayerLevel) {
	 ...
	 * @return An array index used to get an NPC array and a Name array.
	 */
	private int getMazchnaIndex(String previousName, int slayerLevel) {
		int index = r.nextInt(MAZCHNA_MONSTERS.length);
		while (MAZCHNA_NAMES[index] == previousName) { // || slayerLevel >=
														// MAZCHNA_LEVELREQS[index])
														// {
	 ...
	 * @return An array index used to get an NPC array and a Name array.
	 */
	private int getTuraelIndex(String previousName) {
		int index = r.nextInt(TURAEL_MONSTERS.length);
		while (TURAEL_NAMES[index].equals(previousName)) {
	 ...
	 * @return An array index used to get an NPC array and a Name array.
	 */
	private int getVannakaIndex(String previousName, int slayerLevel) {
		return Misc.random(VANNAKA_MONSTERS.length - 1);
	 ...
	 * all thanks.
	 */
	public void setNewTask(Player player, int master) {
		int index = 0;
		int count = 0;
		String taskName = "";
		String lastName = ""; // josh player.getSlayerTask() != null ?
								// player.getSlayerTask().getName() : "";
		switch (master) {
		case TURAEL:
			index = getTuraelIndex(lastName);
		...
		case MAZCHNA:
			// TODO Level check.
			index = getMazchnaIndex(lastName, player.getSkills()
					.getLevelForExperience(Skills.SLAYER));
		...
		case VANNAKA:
			// TODO: Level check.
			index = getVannakaIndex(lastName, player.getSkills()
					.getLevelForExperience(Skills.SLAYER));
			...
			break;
		case CHAELDAR:
			index = getChaeldarIndex(lastName, player.getSkills()
					.getLevelForExperience(Skills.SLAYER));
			...
			break;
		case DURADEL:
			index = getDuradelIndex(lastName, player.getSkills()
					.getLevelForExperience(Skills.SLAYER));
			// int npcIds = -1;
			// npcIds = DURADEL_MONSTERS[index];
			count = getCount(130, 200);
			// final int[] FORTYTOAIGHTY = { 4673, 4674, 4675, 4676, 4418, 1591,
			// 3068, 3069, 3070, 3071, 3590, 4527, 4528, 4529, 4530, 4531,
			// 4532, 4533 };
			/*
			 * 40 - 80 npcs.
			 */
			 ...
			 * Wyvern case 3069://Skeletal Wyvern case 3070://Skeletal Wyvern
			 * case 3071://Skeletal Wyvern case 3590://Steel dragon. case
			 * 4527://Suqah case 4528://Suqah case 4529://Suqah case
			 * 4530://Suqah case 4531://Suqah case 4532://Suqah case
			 * 4533://Suqah for(int npc : npcIds) { for(int i : FORTYTOAIGHTY) {
			 * if(npc == i) { count = getCount(40, 80); break; } } }
			 * 
			 * break;
			 */
		}
		// josh player.setSlayerTask(new SlayerTask(taskName, count, index,
		// master));
	}
}
