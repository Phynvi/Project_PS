...
public class Mining extends ObjectHarvestingAction {
	...
	public static enum Rock {
		 ...
		 * Clay ore.
		 */
		ESS(7936, 1, 5, 1000, -1, new int[] { 2491 }, new int[] { -1 }), CLAY(
				434, 1, 5, 2, 1, new int[] { 2108, 2109 },
				new int[] { 450, 451 }),
		/**
		 * Copper ore.
		 */
		COPPER(436, 1, 17.5, 6, 1, new int[] { 11959, 11960, 11961, 11962,
				2090, 2091 }, new int[] { 9725, 9725, 9725, 9725, 450, 451 }),
		/**
		 * Tin ore.
		 */
		TIN(438, 1, 17.5, 6, 1, new int[] { 2094, 2095, 11957, 11958, 11959 },
				new int[] { 450, 451, 9725, 9725, 9725 }),
		/**
		 * Iron ore.
		 */
		IRON(440, 15, 35, 15, 1, new int[] { 11936, 11937, 11938, 11954, 11955,
				11956, 2092, 2093 }, new int[] { 9725, 9725, 9725, 9725, 9725,
				9725, 450, 451 }),
		/**
		 * Silver ore.
		 */
		SILVER(442, 20, 40, 100, 1, new int[] { 2100, 2101 }, new int[] { 450,
				451 }),
		/**
		 * Gold ore.
		 */
		GOLD(444, 40, 65, 100, 1, new int[] { 2098, 2099 }, new int[] { 450,
				451 }),
		/**
		 * Coal ore.
		 */
		COAL(453, 30, 50, 50, 3, new int[] { 11932, 11930, 2096, 2097 },
				new int[] { 9725, 9725, 450, 451 }),
		/**
		 * Mithril ore.
		 */
		MITHRIL(447, 55, 80, 200, 4, new int[] { 11942, 11944, 2102, 2103 },
				new int[] { 9725, 9725, 450, 451 }),
		/**
		 * Adamantite ore.
		 */
		ADAMANTITE(449, 70, 95, 400, 6, new int[] { 11941, 11939, 2104, 2105 },
				new int[] { 9725, 9725, 450, 451 }),
		/**
		 * Rune ore.
		 */
		RUNE(451, 85, 125, 1000, 8, new int[] { 2106, 2107, 14859, 14860 },
				new int[] { 450, 451, 9725, 9725 });
		/**
		 ...
		 *            The object ids.
		 */
		private Rock(int log, int level, double experience, int respawnTimer,
				int oreCount, int[] objects, int[] replacementRocks) {
	...
	public static int[] getRockIds() {
		return new int[] { 11959, 11960, 11961, 11962, 11957, 11958, 11959,
				11954, 11955, 11956, 11954, 11955, 11956 };
	}
	...
	public boolean canHarvest() {
		if (getGameObject().getRegion() == null) {
			return false;
		}
		for (PickAxe pickaxe : PickAxe.values()) {
			if ((getMob().getInventory().contains(pickaxe.getId()) || getMob()
					.getEquipment().contains(pickaxe.getId()))
					&& getMob().getSkills().getLevelForExperience(getSkill()) >= pickaxe
							.getRequiredLevel()) {
	...
	public int getCycleCount() {
		...
		double cycleCount = 1;
		cycleCount = Math.ceil((level * 50 - skill * 10) / modifier * 0.0625
				* 3);
		if (cycleCount < 1) {
			cycleCount = 1;
		}
	...
	public String getLevelTooLowMessage() {
		return "You need a " + Skills.SKILL_NAME[getSkill()] + " level of "
				+ rock.getRequiredLevel() + " to mine this rock.";
	}
	...
	public GameObject getReplacementObject() {
		...
		for (int i = 0; i < rock.getObjectIds().length; i++) {
			if (rock.getObjectIds()[i] == getGameObject().getDefinition()
					.getId()) {
				index = i;
				break;
			}
		}
		return new GameObject(getGameObject().getLocation(),
				rock.getReplacementRocks()[index], getGameObject().getType(),
				getGameObject().getDirection(), false);
	}
	...
	public String getSuccessfulHarvestMessage() {
		return "You manage to mine some "
				+ getReward().getDefinition().getName().toLowerCase()
						.replaceAll(" ore", "") + ".";
	}
	...
	public String getUnsuccessfulAttemptMessage() {
		return "There is currently no ores remaining in this rock.";
	}
}
