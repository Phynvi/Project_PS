...
public class Agility {
	...
	public enum Obstacle {
		...
		WILDERNESS_DITCH(23271, Location.create(0, 0, 0), 1, 0,
				"wildernessDitch"),

		VARROCK_FENCE(9300, Location.create(0, 0, 0), 1, 0, "varrockFence"),

		FALADOR_WESTERN_CRUMBLING_WALL(11844, Location.create(2935, 3355, 0),
				5, 0.5, "faladorCrumblingWall"),
		...
		GNOME_COURSE_OBSTACLE_PIPE_1(154, Location.create(2484, 3431, 0), 1, 8,
				"gnomeObstaclePipe"),

		GNOME_COURSE_OBSTACLE_PIPE_2(154, Location.create(2487, 3431, 0), 1, 8,
				"gnomeObstaclePipe"),
		...
		BARBARIAN_COURSE_OBSTACLE_PIPE(2287, Location.create(2552, 3559, 0),
				35, 0, "barbarianObstaclePipe"),

		BARBARIAN_COURSE_ROPE_SWING(2282, Location.create(2551, 3550, 0), 35,
				22, "barbarianRopeSwing"),

		BARBARIAN_COURSE_LOG_BALANCE(2294, Location.create(2550, 3546, 0), 35,
				13.7, "barbarianLogBalance"),

		BARBARIAN_COURSE_OBSTACLE_NET(2284, Location.create(2538, 3545, 0), 35,
				8.2, "barbarianObstacleNet"),

		BARBARIAN_COURSE_LEDGE(2302, Location.create(2535, 3547, 1), 35, 22,
				"barbarianLedge"),

		BARBARIAN_COURSE_CRUMBLING_WALL_1(1948, Location.create(2536, 3553, 0),
				35, 13.7, "barbarianCrumblingWall1"),

		BARBARIAN_COURSE_CRUMBLING_WALL_2(1948, Location.create(2539, 3553, 0),
				35, 13.7, "barbarianCrumblingWall2"),

		BARBARIAN_COURSE_CRUMBLING_WALL_3(1948, Location.create(2542, 3553, 0),
				35, 13.7, "barbarianCrumblingWall3"),
		...
		private Obstacle(int id, Location location, int levelRequired,
				double experience, String scriptString) {
			this.id = id;
	...
	public static void animateObject(final GameObject gameObject,
			final Animation animation, int ticks) {
		Tickable tick = new Tickable(ticks) {
			@Override
			public void execute() {
				for (Region r : gameObject.getRegion().getSurroundingRegions()) {
					for (Player player : r.getPlayers()) {
						player.getActionSender().animateObject(gameObject,
								animation.getId());
					}
				}
				this.stop();
	...
	public static void forceMovement(final Player player,
			final Animation animation, final int[] forceMovement, int ticks,
	...
	public static void forceTeleport(final Player player,
			final Animation animation, final Location newLocation,
	...
	public static void forceWalkingQueue(final Player player,
			final Animation animation, final int x, final int y,
			int delayBeforeMovement, final int ticks,
			final boolean removeAttribute) {
		...
		final Animation originalTurn90ccw = player
				.getTurn90CounterClockwiseAnimation();
		final Animation originalTurn180 = player
				.getTurn90CounterClockwiseAnimation();
	...
	public static void setRunningToggled(final Player player, boolean toggled,
			int ticks) {
		final boolean originalToggledState = player.getWalkingQueue()
				.isRunningToggled();
		...
		Tickable tick = new Tickable(ticks) {
			...
			public void execute() {
				player.getWalkingQueue()
						.setRunningToggled(originalToggledState);
				this.stop();
			}
		};
		if (tick.getTickDelay() >= 1) {
	...
	public static void tackleObstacle(Player player, Obstacle obstacle,
			GameObject object) {
		if (player.getSkills().getLevelForExperience(Skills.AGILITY) < obstacle
				.getLevelRequired()) {
		...
		if (ScriptManager.getScriptManager().call(obstacle.getScriptString(),
				player, obstacle, object)) {
		} else {
			;
			player.removeAttribute("busy");
