...
public class FightPits extends AbstractMinigame {
	...
	public FightPits() {
		...
		BoundaryManager
				.addBoundary(Boundary.create("Fight Pits Waiting Room",
						Location.create(2393, 5169, 0),
						Location.create(2406, 5176, 0))); // waiting area
		BoundaryManager.addBoundary(Boundary.create("PvP Zone", getBoundary()
				.getBottomLeft(), getBoundary().getTopRight())); // attackable
																	// area
		BoundaryManager.addBoundary(Boundary.create("MultiCombat",
				getBoundary().getBottomLeft(), getBoundary().getTopRight())); // attackable
																				// area
	...
	public Boundary getBoundary() {
		return Boundary.create(getName(), Location.create(2373, 5126, 0),
				Location.create(2424, 5168, 0));
	}
	...
	public Tickable getGameCycle() {
		return new Tickable(10) {
			...
			public void execute() {
				if (!gameStarted
						&& (waitingRoom.size() + participants.size()) >= FightPits.MINIMUM_SIZE
						&& participants.size() <= 1) {
					start();
				}
			}
	...
	public void killHook(Player player, Mob victim) {
		...
		if (getParticipants().contains(player) && getParticipants().size() <= 1) { // only
																					// us
																					// left
			winGame(player);
		} else {
			for (Player participant : getParticipants()) {
				participant.getActionSender().sendString(373, 1,
						"Foes Remaining: " + (getParticipants().size() - 1));
			}
		}
	}
