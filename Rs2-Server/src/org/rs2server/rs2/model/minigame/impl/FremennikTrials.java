...
public class FremennikTrials extends AbstractMinigame {
	...
	private enum Wave {
		...
		ONE(0, Location.create(2660, 10091, 2), new NPC[] { new NPC(
				NPCDefinition.forId(3697), Location.create(2659, 10091, 2),
				null, null, WalkingQueue.EAST) }),

		TWO(1, Location.create(2646, 10083, 2), new NPC[] { new NPC(
				NPCDefinition.forId(3701), Location.create(2646, 10082, 2),
				null, null, WalkingQueue.NORTH) }),

		THREE(2, Location.create(2647, 10076, 2), new NPC[] {
				new NPC(NPCDefinition.forId(3697), Location.create(2647, 10075,
						2), null, null, WalkingQueue.NORTH),
				new NPC(NPCDefinition.forId(3701), Location.create(2648, 10076,
						2), null, null, WalkingQueue.WEST) }),

		FOUR(3, Location.create(2655, 10072, 2), new NPC[] { new NPC(
				NPCDefinition.forId(3703), Location.create(2655, 10071, 2),
				null, null, WalkingQueue.NORTH) }),

		FIVE(4, Location.create(2664, 10081, 2), new NPC[] {
				new NPC(NPCDefinition.forId(3703), Location.create(2664, 10082,
						2), null, null, WalkingQueue.SOUTH),
				new NPC(NPCDefinition.forId(3701), Location.create(2663, 10081,
						2), null, null, WalkingQueue.EAST),
				new NPC(NPCDefinition.forId(3697), Location.create(2665, 10081,
						2), null, null, WalkingQueue.WEST) });
	...
	public FremennikTrials(Player player) {
		int[] equipment = new int[] { Equipment.SLOT_HELM, Equipment.SLOT_CAPE,
				Equipment.SLOT_AMULET, Equipment.SLOT_CHEST,
				Equipment.SLOT_BOTTOMS, Equipment.SLOT_BOOTS,
				Equipment.SLOT_GLOVES, Equipment.SLOT_RING };
		for (int i = 0; i < equipment.length; i++) {
			if (player.getEquipment().get(equipment[i]) != null) {
				player.getActionSender()
						.sendMessage(
								"You can only take a weapon, shield or ammunition into the Fremennik Trials minigame.");
				return;
			}
		}
	...
	public Boundary getBoundary() {
		return Boundary.create(getName(), Location.create(2638, 10061, 0),
				Location.create(2686, 10109, 0));
	}
			...
			if (currentNPCs.size() < 1) {
				...
				if (wave == null) {
					...
					for (Player participant : participants) {
						...
						NPC skulgrimmen = (NPC) World.getWorld().getNPCs()
								.get(194);
						participant.setInteractingEntity(InteractionMode.TALK,
								skulgrimmen);
						skulgrimmen.setInteractingEntity(InteractionMode.TALK,
								player);
				...
				for (Player participant : participants) {
					participant.setTeleportTarget(Location.create(wave
							.getLocation().getX(), wave.getLocation().getY(),
							heightLvl));
				}
				spawnWave(wave);
			}
	...
	public void spawnWave(Wave wave) {
		...
		for (NPC npc : waveNPCs) {
			NPC newNPC = new NPC(npc.getDefinition(), Location.create(npc
	...
	public void start() {
		...
		for (Player player : participants) {
			...
			player.setTeleportTarget(Location.create(wave.getLocation().getX(),
					wave.getLocation().getY(), heightLvl));
		}
		spawnWave(wave);
	}
