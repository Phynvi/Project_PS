 ...
 *
 * Before you create a field (variable) or method in this class, which is
 * specific to a particular skill, quest, minigame, etc, THINK! There is almost
 * always a better way (e.g. attribute system, helper methods in other classes,
 * etc.)
 *
 * We don't want this to turn into another client.java! If you need advice on
 * alternative methods, feel free to discuss it with me.
 *
 * Graham
 */
...
public abstract class Mob extends Entity {
	...
	public Mob() {
		...
		if (combatState.getPoisonDamage() > 0) {
			World.getWorld().submit(
					this.poisonDrainTick = new PoisonDrainTick(this));
		}
		if (this.combatState.getSpecialEnergy() < 100) {
			World.getWorld()
					.submit(this.specialUpdateTick = new SpecialEnergyRestoreTick(
							this));
		}
	}
	 ...
	 *            The action.
	 */
	public void addCoordinateAction(int width, int height, Location location,
			int otherWidth, int otherHeight, int distance, Action action) {
		actionQueue.clearAllActions();
		if (getLocation().isWithinDistance(width, height, location, otherWidth,
				otherHeight, distance)) {
			actionQueue.addAction(action);
		} else {
			actionQueue.addAction(new CoordinateAction(this, width, height,
					location, otherWidth, otherHeight, distance, action));
		}
	}
	...
	/**
	 * Gets the hit flag defined by the entity type (EG Players wilderness
	 * level)
	 * 
	 * @param victim
	 *            The victim.
	...
	public boolean canTeleport() {
		...
		if (BoundaryManager.isWithinBoundaryNoZ(getLocation(), "PvP Zone")) {
			// TODO : 20- can teleport 21+ cannot
		}
		return true;
	}
	...
	public void inflictDamage(Hit hit, Mob attacker) {
		...
		// if(!combatState.canTeleport() && !(getActionSender() != null &&
		// ((Player) this) != null
		// && ((Player) this).getRequestManager().getRequestType() ==
		// RequestType.DUEL
		// && ((Player) this).getRequestManager().getState() ==
		// RequestState.ACTIVE)) {
		// return;
		// }
		...
		if (combatState.getPrayer(Prayers.REDEMPTION)
			...
			if (getSkills().getLevel(Skills.HITPOINTS) < (getSkills()
					.getLevelForExperience(Skills.HITPOINTS) * 0.10)) {
				...
				getSkills()
						.increaseLevel(
								Skills.HITPOINTS,
								(int) (getSkills().getLevelForExperience(
										Skills.PRAYER) * 0.25));
		...
		getHitQueue().add(hit);
		if (attacker != null) {
			getCombatState().getDamageMap().incrementTotalDamage(attacker,
					hit.getDamage());
			if (!BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(),
					World.getWorld().getFightPits().getBoundary())
					&& !attacker.getCombatState().getDamageMap()
							.getTotalDamages().containsKey(this)
		...
		if (getSkills().getLevel(Skills.HITPOINTS) <= 0
				&& !combatState.isDead()) {
			if (getMob().isPlayer() && getMob().getAttribute("sneaky") != null) {
				if (getActionSender() != null) {
					getMob().getSkills().increaseLevelToMaximum(
							Skills.HITPOINTS,
							getMob().getSkills().getLevelForExperience(
									Skills.HITPOINTS));
				}
			} else {
						...
						public void execute() {
							for (Mob mob : getRegion().getMobs()) {
								if (!mob.getCombatState().isDead()) {
									if (combatState.getLastHitTimer() > (System
											.currentTimeMillis() + 4000)) { // 10
																			// cycles
																			// for
																			// tagging
																			// timer
										if (combatState.getLastHitBy() != null
												&& mob != combatState
														.getLastHitBy()) {
											continue;
										}
									}
									if (mob.getCombatState().getLastHitTimer() > (System
											.currentTimeMillis() + 4000)) { // 10
																			// cycles
																			// for
																			// tagging
									...
									if (mob.getLocation().isNextTo(
											getLocation())
											&& !locationsUsed.contains(mob
													.getLocation())) {
										locationsUsed.add(mob.getLocation());
										int dmg = random
												.nextInt((int) (getSkills()
														.getLevelForExperience(
																Skills.PRAYER) * 0.25)); // +1
																							// as
																							// its
																							// exclusive
	...
	public void removeAllInterfaceAttributes() {
		if (interfaceAttributes != null && interfaceAttributes.size() > 0
				&& interfaceAttributes.keySet().size() > 0) {
	...
	public void resetInteractingEntity() {
		if (this.getInteractingEntity() != null) {
			if (this.getInteractionMode() == InteractionMode.TALK
				...
				this.getInteractingEntity().setInteractingEntity(null, null); // stop
																				// an
																				// infinite
																				// for
																				// loop
	...
	public void setForceWalk(final int[] forceWalk,
		...
		if (forceWalk.length > 0) {
			World.getWorld().submit(new Tickable(forceWalk[7]) {
				...
				@Override
				public void execute() {
					setTeleportTarget(getLocation().transform(forceWalk[2],
							forceWalk[3], 0));
					if (removeAttribute) {
	...
	public void setLocation(Location location) {
		...
		if (isPlayer()) {
			...
			if (player.isActive()) {
				...
				player.getLocation().setLastStep(before.getX(), before.getY(),
						before.getZ());
