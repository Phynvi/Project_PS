...
public class Prayers {
	...
	public static enum Prayer {
		 ...
		 * Burst of Strength.
		 */
		BURST_OF_STRENGTH(Prayers.BURST_OF_STRENGTH, "Burst of Strength", 4, 0,
				84, 12),

		/**
		 * Clarity of Thought.
		 */
		CLARITY_OF_THOUGHT(Prayers.CLARITY_OF_THOUGHT, "Clarity of Thought", 7,
				0, 85, 12),
		 ...
		 * Superhuman Strength.
		 */
		SUPERHUMAN_STRENGTH(Prayers.SUPERHUMAN_STRENGTH, "Superhuman Strength",
				13, 0, 87, 6),

		/**
		 * Improved Reflexes.
		 */
		IMPROVED_REFLEXES(Prayers.IMPROVED_REFLEXES, "Improved Reflexes", 16,
				0, 88, 6),
		 ...
		 * Ultimate Strength.
		 */
		ULTIMATE_STRENGTH(Prayers.ULTIMATE_STRENGTH, "Ultimate Strength", 31,
				0, 93, 3),

		/**
		 * Incredible Reflexes.
		 */
		INCREDIBLE_REFLEXES(Prayers.INCREDIBLE_REFLEXES, "Incredible Reflexes",
				34, 0, 94, 3),

		/**
		 * Protect from Magic.
		 */
		PROTECT_FROM_MAGIC(Prayers.PROTECT_FROM_MAGIC, "Protect from Magic",
				37, 2, 95, 3),

		/**
		 * Protect from Missiles.
		 */
		PROTECT_FROM_MISSILES(Prayers.PROTECT_FROM_MISSILES,
				"Protect from Missiles", 40, 1, 96, 3),

		/**
		 * Protect from Melee.
		 */
		PROTECT_FROM_MELEE(Prayers.PROTECT_FROM_MELEE, "Protect from Melee",
				43, 0, 97, 3),
		 ...
		 * @return
		 */
		private Prayer(int id, String name, int level, int icon, int config,
				double drain) {
			this.id = id;
			this.name = name;
			this.level = level;
	...
	public static void activatePrayer(Player player, int id) {
		if (player.getCombatState().isDead()) {
			player.getActionSender().sendConfig(
					Prayer.forId(id).getClientConfiguration(),
					player.getCombatState().getPrayer(id) ? 1 : 0);
			return;
		}
		...
		if (BoundaryManager.isWithinBoundaryNoZ(player.getLocation(),
				"Fremennik Trials")) {
			player.getActionSender().sendConfig(
					Prayer.forId(id).getClientConfiguration(), 0);
			action.sendDialogue("", DialogueType.MESSAGE, -1,
					FacialAnimation.DEFAULT,
					"Skulgrimen's magic prevents you from using prayers.");
			return;
		}
		Prayer prayer = Prayer.forId(id);
		action.removeAllInterfaces();
		if (player.getSkills().getLevelForExperience(Skills.PRAYER) < prayer
				.getLevelRequired()) {
			action.sendDialogue(
					"",
					...
					-1,
					FacialAnimation.DEFAULT,
					"You need a <col=FF>Prayer level of "
							+ prayer.getLevelRequired() + " to use "
		...
		if (id == CHIVALRY) {
			if (player.getSkills().getLevelForExperience(Skills.DEFENCE) < 65) {
				action.sendDialogue("", DialogueType.MESSAGE, -1,
						FacialAnimation.DEFAULT,
						"You need a <col=FF>Defence level of 65 to use "
								+ prayer.getPrayerName() + ".");
				return;
			}
		}
		if (id == PIETY) {
			if (player.getSkills().getLevelForExperience(Skills.DEFENCE) < 70) {
				action.sendDialogue("", DialogueType.MESSAGE, -1,
						FacialAnimation.DEFAULT,
						"You need a <col=FF>Defence level of 70 to use "
								+ prayer.getPrayerName() + ".");
				return;
			}
		}
		player.getCombatState().setPrayer(id,
				!player.getCombatState().getPrayer(id));
		...
		if (player.getCombatState().getPrayer(id)) {
			...
			for (int i = 0; i < player.getCombatState().getPrayers().length; i++) {
				...
				if (player.getCombatState().getPrayer(i)) {
					player.getActionSender().sendConfig(
							prayer2.getClientConfiguration(), 1);
				} else {
					player.getActionSender().sendConfig(
							prayer2.getClientConfiguration(), 0);
				}
			}
		} else {
			switch (id) {
			...
			player.getActionSender().sendConfig(
					prayer.getClientConfiguration(), 0);
		}
	}
