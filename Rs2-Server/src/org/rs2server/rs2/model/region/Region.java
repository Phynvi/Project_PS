...
public class Region {
	...
	public Door doorForLocation(Location location, int id) {
		for (Door door : doors) {
			if (door.getObject().getLocation().equals(location)
					&& door.getObject().getId() == id
					|| door.getReplacement().getLocation().equals(location)
					&& door.getReplacement().getId() == id) {
				return door;
			}
		}
		return null;
	...
	public Collection<Mob> getMobs() {
		synchronized (this) {
			return Collections
					.unmodifiableCollection(new LinkedList<Mob>(mobs));
		}
	}
	...
	public Region[] getSurroundingRegions() {
		...
		surrounding[1] = World
				.getWorld()
				.getRegionManager()
				.getRegion(this.getCoordinates().getX() - 1,
		...
		surrounding[4] = World
				.getWorld()
				.getRegionManager()
				.getRegion(this.getCoordinates().getX(),
						this.getCoordinates().getY() - 1);
		surrounding[5] = World
				.getWorld()
				.getRegionManager()
