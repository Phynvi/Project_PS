...
public class RequestManager {
	...
	public void cancelRequest() {
		if (state != RequestState.NORMAL) {
			...
			requestType.getRequestListener().requestCancelled(player,
					acquaintance);
			requestType = null;
			acquaintance = null;
		}
	...
	public void finishRequest() {
		if (state != RequestState.FINISHED) {
			...
			if (acquaintance.getRequestManager().acquaintance == player
					&& acquaintance.getRequestManager().requestType == requestType) {
				acquaintance.getRequestManager().setState(RequestState.NORMAL);// we
																				// don't
																				// want
																				// the
																				// acquaintance
			...
			requestType.getRequestListener().requestFinished(player,
					acquaintance);
		}
	}
	...
	public void request(RequestType type, Player acquaintance) {
		if (acquaintance.getInterfaceState().getCurrentInterface() > -1
			...
			this.player.getActionSender().sendMessage(
		...
		if (acquaintance.getRequestManager().acquaintance == player
			...
			this.requestType.getRequestListener().requestAccepted(player,
					acquaintance);
		} else if (this.state != RequestState.PARTICIPATING) {
			this.player.getActionSender().sendMessage(
					"Sending " + type.getClientName().replace("req", "")
							+ " offer...");
			this.acquaintance.getActionSender().sendMessage(
					player.getName() + ":" + type.getClientName() + ":");
		}
	}
