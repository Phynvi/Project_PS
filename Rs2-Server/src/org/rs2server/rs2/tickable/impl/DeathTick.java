...
public class DeathTick extends Tickable {
	...
	public void execute() {
		this.stop();
		if (mob.getCombatState().isDead()) { // they might of been saved in this
												// period
			/*
			...
			/*
			 * Resets the opponents tag timer. Player only as NPC's reset their
			 * killer as soon as they die.
			 */
			if (!mob.isNPC()
					&& mob.getCombatState().getLastHitBy() != null
			...
			if (mob.isPlayer()) {
				...
				if (killer.isPlayer()) {
					...
					if (npc.getDefinition().getId() >= 2025
							&& npc.getDefinition().getId() <= 2030) {
						Barrows.getBarrows().killBrother(player, npc);
				...
				if (npc.getCombatDefinition().getRespawnTicks() > 0) {
					...
					World.getWorld().submit(
							new Tickable(npc.getCombatDefinition()
									.getRespawnTicks()) {
								@Override
								public void execute() {
									npc.setTeleportTarget(npc
											.getSpawnLocation());
