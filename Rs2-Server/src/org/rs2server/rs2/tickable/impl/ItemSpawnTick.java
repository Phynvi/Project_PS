...
public class ItemSpawnTick extends Tickable {
	...
	public void execute() {
		for (ItemSpawn itemSpawn : ItemSpawn.getSpawns()) {
			...
			if (!r.getTile(itemSpawn.getLocation()).getGroundItems()
					.contains(itemSpawn.getGroundItem())) {
				GroundItem newItem = new GroundItem("", itemSpawn.getItem(),
						itemSpawn.getLocation());
