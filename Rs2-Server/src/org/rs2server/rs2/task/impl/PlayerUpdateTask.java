...
public class PlayerUpdateTask implements Task {
	...
	private void appendForceMovement(PacketBuilder packet,
		...
		packet.putByteC((byte) (location.getLocalX(myLocation) + otherPlayer
				.getForceWalk()[0])); // first x to go to
		packet.putByteS((byte) (location.getLocalY(myLocation) + otherPlayer
				.getForceWalk()[1])); // first y to go to
		packet.putByteS((byte) (location.getLocalX(myLocation) + otherPlayer
				.getForceWalk()[2])); // second x to go to
		packet.putByteC((byte) (location.getLocalY(myLocation) + otherPlayer
				.getForceWalk()[3])); // second y to go to
		packet.putShort(otherPlayer.getForceWalk()[4]); // speed going to
														// location
		packet.putShortA(otherPlayer.getForceWalk()[5]); // speed returning to
															// original location
	...
	private void appendHit2Update(PacketBuilder block, Player otherPlayer) {
		...
		block.put((byte) otherPlayer.getSkills().getLevelForExperience(
				Skills.HITPOINTS));
	}
	...
	private void appendHitUpdate(PacketBuilder block, Player otherPlayer) {
		...
		block.putByteC((byte) otherPlayer.getSkills().getLevelForExperience(
				Skills.HITPOINTS));
	}
