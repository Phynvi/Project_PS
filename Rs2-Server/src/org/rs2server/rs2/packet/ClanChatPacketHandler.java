...
public class ClanChatPacketHandler implements PacketHandler {
	...
	public void handle(Player player, Packet packet) {
		...
		case 163:
			if (player.getInterfaceState().getClan().length() < 1) {
				player.getActionSender().sendMessage(
						"Attempting to join channel...:clan:");
				nameAsLong = packet.getLong();
				String name = NameUtils.formatName(NameUtils
				...
				World.getWorld().getPrivateChat().get(name)
						.addClanMember(player);
			} else {
				player.getActionSender().sendMessage(
						"You left the channel.:clan:");
		...
		case 250:
			...
			player.getPrivateChat().getFriends()
					.put(nameAsLong, ClanRank.forId(rank));
		...
		case 22:
			...
			if (!player.getName().equals(privateChat.getOwner())) {
				if (!privateChat.getFriends().containsKey(
						player.getNameAsLong())
						|| (privateChat.getFriends()
								.get(player.getNameAsLong()).getId() < privateChat
								.getKickRank().getId())) {
					player.getActionSender()
							.sendMessage(
									"You do not have a high enough rank to kick in this clan channel.:clan:");
					return;
				}
				if (privateChat.getFriends().containsKey(nameAsLong)) {
					if (privateChat.getFriends().get(nameAsLong).getId() > privateChat
							.getFriends().get(player.getNameAsLong()).getId()) {
						player.getActionSender()
								.sendMessage(
										"You do not have a high enough to kick that person.:clan:");
						return;
					}
				}
			}
			for (Player p : privateChat.getMembers()) {
				if (p.getNameAsLong() == nameAsLong) {
					...
					p.getActionSender().sendMessage(
							"You have been kicked from the channel.:clan:");
					return;
				}
			}
			break;
