...
public class CloseInterfacePacketHandler implements PacketHandler {
	...
	private static final Logger logger = Logger
			.getLogger(CloseInterfacePacketHandler.class.getName());
	...
	public void handle(Player player, Packet packet) {
		if (packet.getOpcode() == 64) {
			...
			case 51:
				...
				switch (childId) {
				case 6:
					Cooking.getSingleton().start(player, permit.getValue(),
							permit.getLocation(), 1);
					break;
				case 5:
					Cooking.getSingleton().start(player, permit.getValue(),
							permit.getLocation(), 5);
					break;
				case 4:
					break;
				case 3:
					Cooking.getSingleton().start(player, permit.getValue(),
							permit.getLocation(), 28);
					break;
				}
			...
			case 53:
				int productionCount;
				if (player.getInterfaceAttribute("herblore_index") != null
					...
					if (productionCount != -1) {
						switch ((HerbloreType) player
								.getInterfaceAttribute("herblore_type")) {
							...
							player.getActionQueue()
									.addAction(
											new Herblore(
													...
													productionCount,
													PrimaryIngredient
															.forId((Integer) player
																	.getInterfaceAttribute("herblore_index")),
													null,
			...
			case 47:
				if (player.getInterfaceAttribute("fletching_log") != null) {
					...
					case 8:
						player.getInterfaceState().openEnterAmountInterface(
								303, logIndex, -1);
						player.setInterfaceAttribute("fletching_index",
								logIndex);
						break;
					}
					if (productionCount != -1) {
						player.getActionQueue()
								.addAction(
										new Fletching(
												...
												logIndex,
												(Log) player
														.getInterfaceAttribute("fletching_log")));
						player.getActionSender().removeChatboxInterface();
					}
				}
				break;
			case 48:
				if (player.getInterfaceAttribute("crafting_leather") != null) {
					int logIndex = -1;
					productionCount = -1;
					switch (childId) {
					case 8:
					...
					case 13:
						player.getInterfaceState().openEnterAmountInterface(
								304, logIndex, -1);
						player.setInterfaceAttribute("crafting_index", logIndex);
						break;
					}
					Leather l = player
							.getInterfaceAttribute("crafting_leather");
					Leather leather = Crafting.getDragonLeather(logIndex,
							l.getLeather());
					if (productionCount != -1 && leather != null) {
						player.getActionQueue().addAction(
								new Crafting(player, null, leather,
										productionCount));
						player.getActionSender().removeChatboxInterface();
					}
				}
			...
			case 49:
				...
				break;
			default:
				logger.info("Unhandled close interface : " + interfaceId
						+ " - " + childId);
				break;
			}
		} else {
