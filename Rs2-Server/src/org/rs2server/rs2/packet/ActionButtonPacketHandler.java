...
public class ActionButtonPacketHandler implements PacketHandler {
	...
	public void handle(final Player player, Packet packet) {
		...
		player.getActionSender().sendDebugPacket(
				packet.getOpcode(),
				"ActionButton",
				new Object[] { "Interface: " + interfaceId,
						"Button: " + button, "ChildButton: " + childButton });
		...
		case 378:
			...
			case 6:
				...
				File f = new File("data/savedGames/"
						...
						+ ".dat.gz");
				if (!f.exists()) {
					ScriptManager.getScriptManager().call("initial_login",
							player);
				}
				return;
			}
		...
		if (SpellBook.forId(player.getCombatState().getSpellBook())
				.getInterfaceId() == interfaceId) {
			Spell spell = Spell.forId(button,
		...
		Shop shop = player.getInterfaceState().getOpenShop() != -1 ? Shop
				.forId(player.getInterfaceState().getOpenShop()) : null;
		...
		case 464:
			if (!player.canAnimate() || !player.canEmote()) {
				player.getActionSender().sendMessage(
						"You can't perform that emote right now.");
				return;
			}
			Tickable emotes = new Tickable(2) { // why a tickable? because if
				...
				public void execute() {
					...
					if (emote != null) {
						...
						case 39:
							if (player.getEquipment().get(Equipment.SLOT_CAPE) == null) {
								...
								player.getActionSender()
										.sendMessage(
												"You aren't wearing a cape of achievement.");
								return;
							}
							SkillCape skillCape = SkillCape.forId(player
									.getEquipment().get(Equipment.SLOT_CAPE));
							if (skillCape == null) {
								player.setEmote(true);
								player.getActionSender()
										.sendMessage(
								...
								if (skillCape.getAnimation() != null) {
									player.playAnimation(skillCape
											.getAnimation());
								}
						...
						default:
							player.setEmote(true);
							logger.info("Unhandled action button : "
									+ interfaceId + " - " + button + " - "
									+ childButton);
							break;
						}
			...
			if (player.getSprites().getPrimarySprite() != -1
					|| player.getSprites().getSecondarySprite() != -1) {
		...
		case 261:
			...
			case 0:
				...
				break;
			case 1:
				player.getSettings().setChatEffects(
						!player.getSettings().chatEffects());
				player.getActionSender().updateChatEffectsConfig();
				break;
			case 2:
				player.getSettings().setSplitPrivateChat(
						!player.getSettings().splitPrivateChat());
				player.getActionSender().updateSplitPrivateChatConfig();
				break;
			case 3:
				player.getSettings().setTwoMouseButtons(
						!player.getSettings().twoMouseButtons());
				player.getActionSender().updateMouseButtonsConfig();
				break;
			case 4:
				player.getSettings().setAcceptAid(
						!player.getSettings().isAcceptingAid());
		...
		case 102:
			...
			case 18: // Kept items
				if (childButton >= 0
						&& childButton < itemsKeptOnDeath[0].size()) {
			...
			case 21: // Lost items
				if (childButton >= 0
						&& childButton < itemsKeptOnDeath[1].size()) {
		...
		case Equipment.INTERFACE:
			...
			case 51:
				...
				player.getActionSender()
						.sendRunScript(150, Constants.EQUIPMENT_PARAMETERS,
								Constants.EQUIPMENT_TYPE_STRING)
		...
		case 92:
			...
			case 24: // Auto retaliate
				player.getSettings().setAutoRetaliate(
		...
		case 93:
			...
			case 8: // Special bar
				...
				if (player.getActiveCombatAction() != null
						&& player.getActiveCombatAction().canSpecial(player,
								player.getInteractingEntity())) {
					player.getActiveCombatAction().special(player,
		...
		case 90:
			...
			case 4:
				...
				if (player.getEquipment().get(Equipment.SLOT_WEAPON) == null) {
					player.getActionSender()
							.sendMessage(
									"You must be wielding a staff in order to autocast.");
					return;
				}
				...
				case MODERN_MAGICS:
					if (player.getEquipment().get(Equipment.SLOT_WEAPON)
						...
						player.getActionSender()
								.sendMessage(
										"You can only autocast Ancient Magicks with that.");
						return;
					}
					if (player.getEquipment().get(Equipment.SLOT_WEAPON)
							.getId() == 4170) {
				...
				case ANCIENT_MAGICKS:
					if (player.getEquipment().get(Equipment.SLOT_WEAPON)
						...
						player.getActionSender()
								.sendMessage(
										"You cannot autocast Ancient Magicks with that.");
						return;
					}
				...
				case LUNAR_MAGICS:
					player.getActionSender().sendMessage(
							"You cannot autocast Lunar Magics with that.");
					break;
				}
		 ...
		 */
		/*
		 * case 499: skillMenu = player.getInterfaceAttribute("skillMenu");
		 * if(skillMenu == -1) { return; } switch (button) { case 10: case 11:
		 * case 12: case 13: case 14: case 15: case 16: case 17: case 18: case
		 * 19: case 20: case 21: case 22: case 23:
		 * player.getActionSender().sendConfig(965, (1024 * (button - 10)) +
		 * skillMenu); break; default: logger.info("Unhandled action button : "
		 * + interfaceId + " - " + button + " - " + childButton); break; }
		 * break;
		 */
		/*
		 * Second trade screen
					...
					case 48: // Items on the interface
						if (childButton >= 0 && childButton < Trade.SIZE) {
							...
							if (item != null && item.getDefinition() != null) {
								switch (packet.getOpcode()) {
								case 234:
									Trade.removeItem(player, childButton,
											item.getId(), 1);
									break;
								case 87:
									Trade.removeItem(player, childButton,
								...
								case 37:
									...
									break;
								case 3:
									Trade.removeItem(player, childButton, item
											.getId(), player.getTrade()
											.getCount(item.getId()));
									break;
								case 70:
									player.getInterfaceState()
											.openEnterAmountInterface(
													Trade.TRADE_INVENTORY_INTERFACE,
													childButton, item.getId());
									break;
								}
							}
						}
			...
			default:
				...
				case Equipment.SCREEN:
					if (childButton >= 0 && childButton < Inventory.SIZE) {
						item = player.getInventory().get(childButton);
						if (item != null && item.getDefinition() != null) {
							WieldItemAction action = new WieldItemAction(
									player, item.getId(), childButton, 0);
							action.execute();
						}
					}
					break;
					...
					case 0: // Items on the interface
						if (childButton >= 0 && childButton < Inventory.SIZE) {
							...
							if (item != null && item.getDefinition() != null) {
								switch (packet.getOpcode()) {
								case 234:
									Trade.offerItem(player, childButton,
											item.getId(), 1);
									break;
								case 87:
									Trade.offerItem(player, childButton,
								...
								case 37:
									...
									break;
								case 3:
									Trade.offerItem(player, childButton, item
											.getId(), player.getInventory()
											.getCount(item.getId()));
									break;
								case 70:
									player.getInterfaceState()
											.openEnterAmountInterface(
													Trade.PLAYER_INVENTORY_INTERFACE,
													childButton, item.getId());
									break;
								}
							}
			...
			case 29: // Main stock
				if (shop != null) {
					if (shop.getMainStock().size() < 1) {
						player.getActionSender().sendMessage(
								"This shop doesn't have a main stock.");
					}
					Shop.openShop(player, player.getInterfaceState()
							.getOpenShop(), 1);
				}
				break;
			...
			case 27: // Player stock
				if (shop != null) {
					if (shop.getPlayerStock().size() < 1) {
						player.getActionSender().sendMessage(
								"This shop doesn't have a player stock.");
					}
					Shop.openShop(player, player.getInterfaceState()
							.getOpenShop(), 2);
				}
		...
		case Shop.PLAYER_INVENTORY_INTERFACE:
			switch (button) {
			case 0:
				if (shop != null && childButton >= 0
						&& childButton < Inventory.SIZE) {
					item = player.getInventory().get(childButton);
					if (item != null && item.getDefinition() != null) {
						switch (packet.getOpcode()) {
						case 234:
							...
							if (shop.getShopType() == ShopType.SPECIALIST_STORE_BUY) {
								if (shop.getMainStock().contains(item.getId())
										|| shop.getDefaultStock().contains(
												item.getId())) {
									canSell = true;
								}
							}
							...
							if (canSell) {
								Shop.valueItem(player, childButton,
										item.getId());
							} else {
								player.getActionSender().sendMessage(
						...
						case 70:
							player.getInterfaceState()
									.openEnterAmountInterface(
											Shop.PLAYER_INVENTORY_INTERFACE,
											childButton, item.getId());
							break;
						case 156:
							player.getActionSender().sendMessage(
		...
		case 319:
			switch (button) {
			case 0:
				MagicCombatAction.setAutocast(player, Spell.WIND_STRIKE,
						config, defensive);
				break;
			case 1:
				MagicCombatAction.setAutocast(player, Spell.WATER_STRIKE,
						config, defensive);
				break;
			case 2:
				MagicCombatAction.setAutocast(player, Spell.EARTH_STRIKE,
						config, defensive);
				break;
			case 3:
				MagicCombatAction.setAutocast(player, Spell.FIRE_STRIKE,
						config, defensive);
				break;
			case 4:
				MagicCombatAction.setAutocast(player, Spell.WIND_BOLT, config,
						defensive);
				break;
			case 5:
				MagicCombatAction.setAutocast(player, Spell.WATER_BOLT, config,
						defensive);
				break;
			case 6:
				MagicCombatAction.setAutocast(player, Spell.EARTH_BOLT, config,
						defensive);
				break;
			case 7:
				MagicCombatAction.setAutocast(player, Spell.FIRE_BOLT, config,
						defensive);
				break;
			case 8:
				MagicCombatAction.setAutocast(player, Spell.WIND_BLAST, config,
						defensive);
				break;
			case 9:
				MagicCombatAction.setAutocast(player, Spell.WATER_BLAST,
						config, defensive);
				break;
			case 10:
				MagicCombatAction.setAutocast(player, Spell.EARTH_BLAST,
						config, defensive);
				break;
			case 11:
				MagicCombatAction.setAutocast(player, Spell.FIRE_BLAST, config,
						defensive);
				break;
			case 12:
				MagicCombatAction.setAutocast(player, Spell.WIND_WAVE, config,
						defensive);
				break;
			case 13:
				MagicCombatAction.setAutocast(player, Spell.WATER_WAVE, config,
						defensive);
				break;
			case 14:
				MagicCombatAction.setAutocast(player, Spell.EARTH_WAVE, config,
						defensive);
				break;
			case 15:
				MagicCombatAction.setAutocast(player, Spell.FIRE_WAVE, config,
						defensive);
				break;
			case 174: // Cancel
		...
		case 310:
			switch (button) {
			case 61:
				MagicCombatAction.setAutocast(player, Spell.CRUMBLE_UNDEAD, 77,
						defensive);
				break;
			case 0:
				MagicCombatAction.setAutocast(player, Spell.MAGIC_DART, 79,
						defensive);
				break;
			case 1:
				MagicCombatAction.setAutocast(player, Spell.WIND_WAVE, 69,
						defensive);
				break;
			case 2:
				MagicCombatAction.setAutocast(player, Spell.WATER_WAVE, 71,
						defensive);
				break;
			case 3:
				MagicCombatAction.setAutocast(player, Spell.EARTH_WAVE, 73,
						defensive);
				break;
			case 4:
				MagicCombatAction.setAutocast(player, Spell.FIRE_WAVE, 75,
						defensive);
				break;
			case 49: // Cancel
				player.getEquipment().fireItemChanged(Equipment.SLOT_WEAPON);
		...
		case 388:
			switch (button) {
			case 93:
				MagicCombatAction.setAutocast(player, Spell.SMOKE_RUSH, 13,
						defensive);
				break;
			case 145:
				MagicCombatAction.setAutocast(player, Spell.SHADOW_RUSH, 15,
						defensive);
				break;
			case 51:
				MagicCombatAction.setAutocast(player, Spell.BLOOD_RUSH, 17,
						defensive);
				break;
			case 7:
				MagicCombatAction.setAutocast(player, Spell.ICE_RUSH, 19,
						defensive);
				break;
			case 119:
				MagicCombatAction.setAutocast(player, Spell.SMOKE_BURST, 21,
						defensive);
				break;
			case 171:
				MagicCombatAction.setAutocast(player, Spell.SHADOW_BURST, 23,
						defensive);
				break;
			case 71:
				MagicCombatAction.setAutocast(player, Spell.BLOOD_BURST, 25,
						defensive);
				break;
			case 29:
				MagicCombatAction.setAutocast(player, Spell.ICE_BURST, 27,
						defensive);
				break;
			case 106:
				MagicCombatAction.setAutocast(player, Spell.SMOKE_BLITZ, 29,
						defensive);
				break;
			case 158:
				MagicCombatAction.setAutocast(player, Spell.SHADOW_BLITZ, 31,
						defensive);
				break;
			case 62:
				MagicCombatAction.setAutocast(player, Spell.BLOOD_BLITZ, 33,
						defensive);
				break;
			case 18:
				MagicCombatAction.setAutocast(player, Spell.ICE_BLITZ, 35,
						defensive);
				break;
			case 132:
				MagicCombatAction.setAutocast(player, Spell.SMOKE_BARRAGE, 37,
						defensive);
				break;
			case 184:
				MagicCombatAction.setAutocast(player, Spell.SHADOW_BARRAGE, 39,
						defensive);
				break;
			case 82:
				MagicCombatAction.setAutocast(player, Spell.BLOOD_BARRAGE, 41,
						defensive);
				break;
			case 40:
				MagicCombatAction.setAutocast(player, Spell.ICE_BARRAGE, 43,
						defensive);
				break;
			case 6: // Cancel
				player.getEquipment().fireItemChanged(Equipment.SLOT_WEAPON);
		...
		case 589:
			...
			case 9:
				if (World.getWorld().clanIsRegistered(player.getName())) {
					player.getActionSender().sendString(590, 32,
							player.getPrivateChat().getChannelName());
				}
				player.getActionSender().sendString(590, 33,
						player.getPrivateChat().getEntryRank().getText());
				player.getActionSender().sendString(590, 34,
						player.getPrivateChat().getTalkRank().getText());
				player.getActionSender().sendString(590, 35,
						player.getPrivateChat().getKickRank().getText());
		...
		case 590:
			...
			case 32:
				switch (packet.getOpcode()) {
				case 234:
					player.getInterfaceState().openEnterTextInterface(590,
							"Enter chat prefix:");
					break;
				case 87:
		...
		case 199:
			...
			case 104:
				if (player.getInterfaceAttribute("newBeard") != null) {
					player.getAppearance().setLook(12,
							(Integer) player.getInterfaceAttribute("newBeard"));
				}
				if (player.getInterfaceAttribute("newHairColour") != null) {
					player.getAppearance().setLook(
							1,
							(Integer) player
									.getInterfaceAttribute("newHairColour"));
				}
		...
		case 201:
			...
			case 106:
				if (player.getInterfaceAttribute("newLegs") != null) {
					player.getAppearance().setLook(10,
							(Integer) player.getInterfaceAttribute("newLegs"));
				}
				if (player.getInterfaceAttribute("newLegsColour") != null) {
					player.getAppearance().setLook(
							3,
							(Integer) player
									.getInterfaceAttribute("newLegsColour"));
				}
				player.getUpdateFlags().flag(UpdateFlag.APPEARANCE);
				player.getActionSender().removeAllInterfaces();
				break;
		...
		case 202:
			...
			case 106:
				if (player.getInterfaceAttribute("newBody") != null) {
					player.getAppearance().setLook(7,
							(Integer) player.getInterfaceAttribute("newBody"));
				}
				if (player.getInterfaceAttribute("newArms") != null) {
					player.getAppearance().setLook(8,
							(Integer) player.getInterfaceAttribute("newArms"));
				}
				if (player.getInterfaceAttribute("newBodyColour") != null) {
					player.getAppearance().setLook(
							2,
							(Integer) player
									.getInterfaceAttribute("newBodyColour"));
				}
				player.getUpdateFlags().flag(UpdateFlag.APPEARANCE);
				player.getActionSender().removeAllInterfaces();
				break;
		...
		case 203:
			...
			case 99:
				if (player.getInterfaceAttribute("newHair") != null) {
					player.getAppearance().setLook(6,
							(Integer) player.getInterfaceAttribute("newHair"));
				}
				if (player.getInterfaceAttribute("newHairColour") != null) {
					player.getAppearance().setLook(
							1,
		...
		case 205:
			...
			case 88:
				if (player.getInventory().getCount(995) < 3000) {
					player.getActionSender().sendMessage(
							"You don't have enough coins for a make-over.");
					player.getActionSender().removeAllInterfaces();
					break;
				}
				...
				if (player.getInterfaceAttribute("newSkinColour") != null) {
					player.getAppearance().setLook(
							5,
							(Integer) player
									.getInterfaceAttribute("newSkinColour"));
				}
				if (player.getInterfaceAttribute("newGender") != null) {
					player.getAppearance()
							.setLook(
									0,
									(Integer) player
											.getInterfaceAttribute("newGender"));
				}
