...
public class PlayerOptionPacketHandler implements PacketHandler {
	...
	private static final int OPTION_ATTACK = 96, OPTION_2 = 37, OPTION_3 = 185,
			OPTION_4 = 247, OPTION_SPELL = 35;
	...
	private void option2(Player player, Packet packet) {
		...
		player.getActionSender().sendDebugPacket(packet.getOpcode(), "Opt2",
				new Object[] { "Index: " + id });
	}
	...
	private void option3(Player player, Packet packet) {
		...
		player.getActionSender().sendDebugPacket(packet.getOpcode(), "Follow",
				new Object[] { "Index: " + id });
	...
	private void option4(Player player, Packet packet) {
		...
		player.getActionSender().sendDebugPacket(packet.getOpcode(), "Trade",
				new Object[] { "Index: " + id });
	}
	...
	private void optionAttack(final Player player, Packet packet) {
		...
		player.getActionSender().sendDebugPacket(packet.getOpcode(),
				"PlayerAttack", new Object[] { "Index: " + id });
	}
	...
	private void optionSpell(Player player, Packet packet) {
			...
			if (spell.getSpellType() == SpellType.COMBAT) {
				...
				if (!BoundaryManager.isWithinBoundaryNoZ(player.getLocation(),
					...
					player.getActionSender()
							.sendMessage(
									"You must be in a multi-combat area to cast that spell.");
					return;
				}
				if (!victim.getSettings().isAcceptingAid()) {
					player.getActionSender().sendMessage(
							"That person doesn't have accept aid on.");
					return;
				}
				if (player.getLocation().isWithinDistance(victim.getLocation(),
						10)) {
		...
		player.getActionSender().sendDebugPacket(packet.getOpcode(), "Spell",
				new Object[] { "ChildButton: " + childButton, "Index: " + id });
	}
