...
public abstract class Player {
	...
	public long lastCast = 0, lastDesert, eventTimer;

	public boolean initialized = false, musicOn = true, lucas, playerIsCooking, disconnected = false,
			...
			mouseButton = false, chatEffects = true,
		    ...
		    secondPinEnter, thirdPinEnter, fourthPinEnter, hasBankpin, isSmelting, isBanking, isTeleporting = false;

	public int

			saveDelay, playerKilled, gertCat, restGhost, romeojuliet, runeMist, vampSlayer, cookAss, doricQuest, sheepShear, 
			impsC, randomActions, pkPoints, totalPlayerDamageDealt, killedBy,
			lastChatId = 1, privateChat, friendSlot = 0, dialogueId, randomCoffin, newLocation, specEffect, specBarId, attackLevelReq,
			...
			lastArrowUsed = -1, autoRet = 0, pcDamage = 0,
			...
			killCount = 0, witchspot, pirateTreasure, ptjob, cwKills, cwDeaths, cwGames, 
			tzKekSpawn = 0, playerBankPin, recoveryDelay = 3, attemptsRemaining = 3, lastPinSettings = -1, setPinDate = -1, changePinDate = -1,
		    deletePinDate = -1, firstPin, secondPin, thirdPin, fourthPin, bankPin1, bankPin2, bankPin3, bankPin4, pinDeleteDateRequested,
		    lastLoginDate, selectedSkill, log = -1;
	...
	public int pieSelect = 0, getPheasent, kebabSelect = 0, breadID, chocSelect = 0, bagelSelect = 0, triangleSandwich = 0, squareSandwich = 0, breadSelect = 0;
	...
	public Task skilling;

	public void setSkilling(Task event) {
		this.skilling = event;
	}

	public Task getSkilling() {
		return skilling;
	}
	...
	public int npcIndex, npcClickIndex, npcType, castingSpellId, oldSpellId, spellId,
			hitDelay;
    ...
    public boolean FightPitsArea() {
        return (absX >= 2378 && absX <= 2415 && absY >= 5133 && absY <= 5167) || (absX >= 2394 && absX <= 2404 && absY >= 5169 && absY <= 5174);
    }
	...
	public Player(int _playerId) {
		...
		teleportToX = TutorialIsland.TUTORIAL_X;
		teleportToY = TutorialIsland.TUTORIAL_Y;
