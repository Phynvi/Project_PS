...
public class Smithing {
	...
	public void readInput(int level, String type, Client c, int amounttomake) {
		...
		if (c.getItems().getItemName(Integer.parseInt(type)).contains("Bronze") && c.getItems().playerHasItem(2347, 1))
		{
			CheckBronze(c, level, amounttomake, type);
		}
		else if (c.getItems().getItemName(Integer.parseInt(type)).contains("Iron") && c.getItems().playerHasItem(2347, 1))
		{
			CheckIron(c, level, amounttomake, type);
		}
		else if (c.getItems().getItemName(Integer.parseInt(type)).contains("Steel") && c.getItems().playerHasItem(2347, 1))
		{
			CheckSteel(c, level, amounttomake, type);
		}
		else if (c.getItems().getItemName(Integer.parseInt(type)).contains("Mith") && c.getItems().playerHasItem(2347, 1))
		{
			CheckMith(c, level, amounttomake, type);
		}
		else if (c.getItems().getItemName(Integer.parseInt(type)).contains("Adam") || c.getItems().getItemName(Integer.parseInt(type)).contains("Addy") && c.getItems().playerHasItem(2347, 1))
		{
			CheckAddy(c, level, amounttomake, type);
		}
		else if (c.getItems().getItemName(Integer.parseInt(type)).contains("Rune") || c.getItems().getItemName(Integer.parseInt(type)).contains("Runite") && c.getItems().playerHasItem(2347, 1))
		{
...
remove = 2361;
	...
	public boolean doaction(Client c, int toadd, int toremove, int toremove2, int timestomake, int NOTUSED, int NOTUSED2, int xp) {
		...
		c.getPA().addSkillXP(xp * SkillHandler.SMITHING_EXPERIENCE, 13);
