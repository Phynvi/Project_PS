package server.game.content.clipping.clip.region;
...
public class Region {
	...
	public static void addObject(int objectId, int x, int y, int height,
		...
		if (type == 22) {
			...
			if (def.aBoolean767()) {
				addClippingForSolidObject(x, y, height, xLength, yLength, def
						.solid());
			}
		} else if (type >= 0 && type <= 3) {
			if (def.aBoolean767()) {
				addClippingForVariableObject(x, y, height, type, direction, def
						.solid());
			}
		}
	}
