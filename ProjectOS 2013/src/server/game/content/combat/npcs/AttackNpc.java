	...
	public static void attackNpc(Client c, int i) {
		...
		if (NPCHandler.npcs[i] != null) {
			...
			if (c.attackTimer <= 0) {
				...
				if (c.getCombat().armaNpc(i) && !usingCross && !usingBow && !c.usingMagic
						&& !c.getCombat().usingCrystalBow() && !usingOtherRangeWeapons) {
					c.getCombat().resetPlayerAttack();
					return;
				}
				if ((!c.goodDistance(c.getX(), c.getY(),
						NPCHandler.npcs[i].getX(),
						NPCHandler.npcs[i].getY(), 2) && (c.getCombat().usingHally()
						...
						|| (!c.goodDistance(c.getX(), c.getY(),
								...
								&& !c.getCombat().usingHally() && !usingBow && !c.usingMagic))
						|| ((!c.goodDistance(c.getX(), c.getY(),
								NPCHandler.npcs[i].getX(),
