	...
	private static void whatSkillAreWeLookingFor(Client c, int actionButtonId) {
		for (int[] genie : genieData) {
			if (genie[0] == actionButtonId) {
				if (c.getItems().playerHasItem(GENIE_LAMP1, 1)) {
					c.getItems().deleteItem(GENIE_LAMP1, 1);
					expAmount = 10 * c.playerLevel[genie[1]];
					c.getPA().addSkillXP(expAmount, genie[1]);
					c.getDH().sendStatement("You gained " + expAmount * Config.SERVER_EXP_BONUS + " experience.");
					c.nextChat = 0;
					break;
				} else if (c.getItems().playerHasItem(GENIE_LAMP2, 1)) { 
					c.getItems().deleteItem(GENIE_LAMP2, 1);
					expAmount = 10 * c.playerLevel[genie[1]];
					c.getPA().addSkillXP(expAmount, genie[1]);
					c.getDH().sendStatement("You gained " + expAmount * Config.SERVER_EXP_BONUS + " experience.");
					c.nextChat = 0;
